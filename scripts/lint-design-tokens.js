...
for (const file of files) {
    ...
    if (!isTokenFile) {
        ...
        if (hexMatches) {
            violations.push(`${path}: Raw hex colors in component code: ${hexMatches.join(", ")}`);
        }
    }
    ...
    if (concatAlpha) {
        violations.push(`${path}: Use alpha(color, N) instead of string concat: ${concatAlpha.join(", ")}`);
    }
    ...
    if (cssVarMatches) {
        ...
        if (badVars.length) {
            violations.push(`${path}: Unregistered CSS variables: ${badVars.join(", ")}`);
        }
    }
    // ── 4. Direct primitive import (bypasses semantic layer) ────────────────
    if (text.includes("from './primitive'") ||
        text.includes('from "./primitive"') ||
        text.includes("from './tokens/primitive'")) {
        // Allow in semantic.ts and component.ts
        if (!isTokenFile) {
            violations.push(`${path}: Importing from primitive layer directly — use semantic tokens instead`);
        }
    }
    ...
    if (suspiciousStyle && !isTokenFile) {
        ...
        const bad = suspiciousStyle.filter((s) => /color:\s*['"][^'"]*(red|blue|green|white|black|#[0-9a-fA-F])/.test(s));
        if (bad.length) {
            violations.push(`${path}: Raw color strings in style object: ${bad.join(", ")}`);
        }
    }
}
