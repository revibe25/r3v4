 ...
 */
import { scoreTransition, rankTransitions, DEFAULT_WEIGHTS, WEIGHT_PROFILES, } from '@llpte/llpte-transition-graph';
// ── Utilities ─────────────────────────────────────────────────────────────────
function now() {
...
function benchSingleScoring(iterations = 10000) {
...
function benchGraphRanking() {
    ...
    for (const size of sizes) {
        ...
        // Warm up
        for (let i = 0; i < 5; i++)
            rankTransitions(source, 'track-0', rest);
        for (let i = 0; i < 200; i++) {
            const t0 = now();
        ...
        row(`Library size ${size.toString().padStart(5)} tracks`, `mean ${fmt(mean(times))}  p99 ${fmt(p99(times))}  ${pass ? '✅' : '❌'}`);
    }
    row('Target', '< 10ms for ≤100 tracks, < 50ms for ≤1000 tracks');
}
// ── Benchmark 3 — Weight profile comparison ───────────────────────────────────
function benchWeightProfiles(iterations = 5000) {
    ...
    for (const [name, weights] of Object.entries(WEIGHT_PROFILES)) {
        ...
        for (let i = 0; i < iterations; i++) {
            const t0 = now();
            scoreTransition(a, b, 'a', 'b', weights);
            times.push(now() - t0);
        }
...
function benchEndToEnd(iterations = 5000) {
