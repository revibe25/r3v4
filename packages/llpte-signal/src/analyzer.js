...
function computeMetrics(channels) {
    const len = channels[0].length;
    const nCh = channels.length;
    let sum = 0, peak = 0;
    for (let i = 0; i < len; i++) {
...
function computeSpectralCentroid(mono, sampleRate) {
...
function estimateKey(mono, sampleRate) {
    ...
    for (let root = 0; root < 12; root++) {
        ...
        for (const [profile, scale] of [[maj, 'major'], [min, 'minor']]) {
...
export async function analyzeAudio(buffer) {
    ...
    if (buffer.sourceId)
        analysisCache.set(buffer.sourceId, result);
