...
| C-01 | esbuild ≤0.24.2 transitive via @esbuild-kit (GHSA-67mh-4wv8-2f99) | `package.json` pnpm.overrides | ✅ Fixed — `"esbuild": ">=0.25.0"` override applied; cleared in `pnpm audit` 2026-04-22 |
...
**Resolved 2026-04-22:** `pnpm.overrides` applied, `pnpm install` run, `pnpm audit` confirms esbuild no longer flagged. Entry retained for audit trail only.
...
- **Owner:** @3R
- **Upgrade path:** Vite 5 → 6 migration targeting `>=6.4.2`. Track upstream changelog for breaking changes. Test client HMR, AudioWorklet worker imports, and any Vite plugins used.
...
### NEW-01 — GHSA-gh4j-gqv2-49f6 · fast-xml-parser <5.7.0 XML injection (transitive via @aws-sdk)

- **Status:** Deferred
- **Advisory status:** Public (N-day)
- **Advisory published:** 2026-04-22 (date first observed in audit)
- **Surface:** Runtime (transitive) — path is `.>@aws-sdk/client-s3>@aws-sdk/core>@aws-sdk/xml-builder>fast-xml-parser`. S3 is called at runtime but no user-supplied input flows into the XML builder.
- **Our severity:** Low — S3 keys are server-generated (`samples/${uuidv4()}.${ext}`); file extension is regex-sanitized before use (`/[^.a-z0-9]/gi` stripped in `loops.ts`). No attacker-controlled string reaches the XML serialization path.
- **Advisory severity:** Moderate — delta: -1 (our implementation does not expose the vulnerable input path).
- **Mythos-class re-price:** Even with model-assisted fuzzing, the injection path requires user input to reach the XML builder. It does not.
- **Why deferred:** No exploitable path in current implementation. Fix is a routine `@aws-sdk` bump.
- **Interim control:** Server-generated S3 keys with sanitized extensions act as a barrier — no user input reaches the XML builder.
- **Revisit trigger:** 2026-07-22 or next `@aws-sdk` bump, whichever comes first.
- **Owner:** @3R
- **Upgrade path:** Bump `@aws-sdk` packages to pull in `fast-xml-parser >= 5.7.0` transitively.

---

### NEW-02 — GHSA-w5hq-g745-h8pq · uuid <14.0.0 buffer bounds check missing in v3/v5/v6

- **Status:** Deferred
- **Advisory status:** Public (N-day)
- **Advisory published:** 2026-04-22 (date first observed in audit)
- **Surface:** Dev-build-isolated — the vulnerable codepath requires passing a `buf` argument to `v3`, `v5`, or `v6`. Full codebase grep confirms every call is `uuidv4()` with no arguments. The `buf` signature is never invoked.
- **Our severity:** Low — vulnerable signature unused. No attacker-controlled buffer passed anywhere.
- **Advisory severity:** Moderate — delta: -1 (vulnerable signature not called in this codebase).
- **Mythos-class re-price:** Exploitation requires the `buf` parameter to be present in a call. It is not.
- **Why deferred:** No exploitable path. Major version bump (v14) requires testing for behavioral changes in `uuidv4()` output, though none are expected.
- **Interim control:** Unused vulnerable signature is an effective barrier in current code.
- **Revisit trigger:** 2026-07-22 or next dependency audit cycle.
- **Owner:** @3R
- **Upgrade path:** Bump `uuid` to `>=14.0.0`. Verify `uuidv4()` output format unchanged. No breaking changes expected for v4 usage.
