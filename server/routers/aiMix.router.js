...
import { logAIDecision, updateAIDecisionOutcome, } from "../services/session-metrics.service";
const aiService = { analyze: async (_) => ({ suggestions: [] }) };
...
process.stderr.write("[deprecation] server/routers/aiMix.router.ts is loaded but unused. " +
    ...
    analyze: protectedProcedure
        ...
        if (input.sessionId) {
            ...
            Promise.all(logPromises).catch((err) => {
                ...
                process.stderr.write(`[aiMixRouter] aiDecisionLog write failed: ${String(err)}\n`);
            });
        }
        return result;
    }),
