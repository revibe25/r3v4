...
export const trialRouter = router({
  status: protectedProcedure.query(async ({ ctx }) => {
    ...
    if (user.tier && user.tier !== "explorer") {
      return { state: "subscribed" as const, expiresAt: null, daysLeft: null };
    }

    if (!user.trialStartedAt || !user.trialExpiresAt) {
      return { state: "not_started" as const, expiresAt: null, daysLeft: null };
    }
    ...
    return {
      state: daysLeft > 0 ? ("active" as const) : ("expired" as const),
  ...
  activate: protectedProcedure.mutation(async ({ ctx }) => {
    ...
    if (!user) throw new TRPCError({ code: "NOT_FOUND" });
    if (user.tier && user.tier !== "explorer") return { activated: false, reason: "already_subscribed" };
    if (user.trialStartedAt) return { activated: false, reason: "already_activated" };
