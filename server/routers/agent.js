...
const requireAgentToken = middleware(({ ctx, next }) => {
    if (!AGENT_TOKEN) {
        process.stderr.write('[agent-router] AGENT_SERVICE_TOKEN not set — rejecting all agent calls\n');
        throw new TRPCError({
...
export const agentRouter = router({
    decisions: agentProcedure
        ...
        .mutation(({ input }) => {
        process.stderr.write(`[agent-router] decisions agentId=${input.agentId} sessionId=${input.sessionId} count=${input.decisions.length}\n`);
    ...
    mix: agentProcedure
        ...
        .mutation(({ input }) => {
        process.stderr.write(`[agent-router] mix agentId=${input.agentId} sessionId=${input.sessionId}\n`);
        return { ok: true, agentId: input.agentId };
    }),
    diagnostics: agentProcedure
        ...
        process.stderr.write(`[agent-router] diagnostics agentId=${input.agentId} findings=${input.findings.length} critical=${critCount}\n`);
        return { ok: true, agentId: input.agentId };
    }),
    vocalspectra: agentProcedure
        .input(vocalSpectraSchema)
        .mutation(({ input }) => {
        process.stderr.write(`[agent-router] vocalspectra agentId=${input.agentId} trackId=${input.trackId}\n`);
        return { ok: true, agentId: input.agentId };
    }),
});
