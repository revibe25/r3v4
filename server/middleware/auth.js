...
export function trpcAuth(req, _res, next) {
...
export function loopStationAuth(req, res, next) {
...
export async function assertTrialActive(userId) {
    const [user] = await _db
        ...
        .limit(1);
    if (!user)
        throw new _TRPCError({ code: "UNAUTHORIZED" });
    if (user.tier && user.tier !== "explorer")
        return; // subscribed — pass
    if (!user.trialExpiresAt)
        return; // no trial yet — pass (client gates)
