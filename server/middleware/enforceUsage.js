 ...
 */
export async function enforceUsage(req, res, next) {
    ...
    try {
        ...
        const rawTier = user.tier;
        const tier = isPlan(rawTier) ? rawTier : "explorer";
        const limit = MIX_LIMITS[tier] ?? DEFAULT_LIMIT;
        const mixCount = await storage.getMixCountByUser(userId);
        if (mixCount >= limit) {
            logger.warn({ userId, tier, mixCount, limit }, "enforceUsage: mix limit reached");
            res.status(403).json({
