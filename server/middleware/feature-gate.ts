...
const guestCounters = new Map<string, GuestCounter>();

const evictionInterval = setInterval(() => {
  const cutoff = Date.now() - GUEST_TTL_MS;
  for (const [key, entry] of guestCounters) {
    if (entry.lastSeen < cutoff) guestCounters.delete(key);
  }
}, GUEST_TTL_MS);

if (evictionInterval.unref) evictionInterval.unref();

function getGuestCount(sessionId: string): number {
  return guestCounters.get(sessionId)?.count ?? 0;
}

function incrementGuestCount(sessionId: string): void {
  ...
  if (existing) {
    ...
    if (guestCounters.size >= MAX_GUEST_SESSIONS) {
      ...
      if (oldestKey) guestCounters.delete(oldestKey);
    }
