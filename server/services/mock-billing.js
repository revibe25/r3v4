 ...
 */
export function verifyMockToken(token, sig) {
    if (sign(token) !== sig)
        return null;
    try {
...
export function createMockCheckoutUrl(opts) {
...
export function createMockPortalUrl(opts) {
    const encoded = encodeToken({
        userId: opts.userId,
 ...
 */
export async function applyMockSubscription(userId, tier, billingCycle, trialDays = 14) {
    const now = new Date();
    const trialEnd = new Date(now.getTime() + trialDays * 86400000);
    const periodEnd = new Date(now.getTime() + (billingCycle === 'annual' ? 365 : 30) * 86400000);
...
export async function cancelMockSubscription(userId) {
    const existing = await db
        .select({ id: subscriptions.id })
        .from(subscriptions)
        .where(eq(subscriptions.userId, userId))
...
export function renderCheckoutPage(tier, billingCycle, trialDays, token, sig) {
...
export function renderPortalPage(tierName, billingCycle, status, periodEnd, token, sig) {
