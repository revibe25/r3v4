...
function getClient(): S3Client {
  if (!_client) {
    _client = new S3Client({
      region: 'auto',
      endpoint: UPLOAD_CONFIG.R2_ENDPOINT,
      credentials: { accessKeyId: UPLOAD_CONFIG.R2_ACCESS_KEY_ID, secretAccessKey: UPLOAD_CONFIG.R2_SECRET_ACCESS_KEY },
    });
  }
  return _client;
}
...
export async function createSignedUploadUrl(filename: string, contentType: string, size: number): Promise<SignedUploadResult> {
  ...
  const command = new PutObjectCommand({ Bucket: UPLOAD_CONFIG.R2_BUCKET_NAME, Key: fileKey, ContentType: contentType, ContentLength: size });
...
export async function createMultipartUpload(filename: string, contentType: string, size: number): Promise<MultipartSignedResult> {
  const ext = filename.split('.').pop() ?? 'bin';
  const fileKey = `samples/${uuidv4()}.${ext}`;
  const create = await getClient().send(new CreateMultipartUploadCommand({ Bucket: UPLOAD_CONFIG.R2_BUCKET_NAME, Key: fileKey, ContentType: contentType }));
...
export async function getSignedPartUrl(fileKey: string, uploadId: string, partNumber: number) {
  const command = new UploadPartCommand({ Bucket: UPLOAD_CONFIG.R2_BUCKET_NAME, Key: fileKey, UploadId: uploadId, PartNumber: partNumber });
  const uploadUrl = await getSignedUrl(getClient(), command, { expiresIn: UPLOAD_CONFIG.SIGNED_URL_EXPIRES });
...
export async function completeMultipartUpload(fileKey: string, uploadId: string, parts: Array<{ PartNumber: number; ETag: string }>) {
  await getClient().send(new CompleteMultipartUploadCommand({ Bucket: UPLOAD_CONFIG.R2_BUCKET_NAME, Key: fileKey, UploadId: uploadId, MultipartUpload: { Parts: parts } }));
}

export async function abortMultipartUpload(fileKey: string, uploadId: string) {
  await getClient().send(new AbortMultipartUploadCommand({ Bucket: UPLOAD_CONFIG.R2_BUCKET_NAME, Key: fileKey, UploadId: uploadId }));
}

export function getPublicUrl(fileKey: string): string {
  return `${UPLOAD_CONFIG.R2_ENDPOINT.replace(/\/$/, '')}/${UPLOAD_CONFIG.R2_BUCKET_NAME}/${fileKey}`;
}
