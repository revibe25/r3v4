 ...
 */
function normalizeCredentialForLog(credential) {
...
router.post("/register", async (req, res) => {
    ...
    if (!parsed.success) {
        logger.warn({ errors: parsed.error.flatten() }, "register: validation failed");
...
router.post("/login", async (req, res) => {
    ...
    if (!parsed.success) {
        logger.warn({ errors: parsed.error.flatten(), credential: req.body.credential }, "login: validation failed");
        return res.status(400).json({ error: parsed.error.flatten() });
    }
    ...
    try {
        ...
        const passwordValid = await bcrypt.compare(password, storedHash ?? dummyHash);
        ...
        logger.info({ userId: u.id, username: u.username, credential: credentialLog }, "User logged in successfully");
        return res.json({
            token,
            user: safeUser(u),
        });
...
router.post("/change-password", requireUser, async (req, res) => {
    ...
    if (!parsed.success) {
        logger.warn({ errors: parsed.error.flatten() }, "change-password: validation failed");
        return res.status(400).json({ error: parsed.error.flatten() });
    }
