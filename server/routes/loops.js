...
const upload = multer({
    ...
    fileFilter: (_req, file, cb) => ALLOWED_MIMES.has(file.mimetype) ? cb(null, true) : cb(new Error(`Unsupported MIME: ${file.mimetype}`)),
});
...
const buildFilename = (orig) => `${uuidv4()}-${Date.now()}${(path.extname(orig).toLowerCase() || '.wav').replace(/[^.a-z0-9]/gi, '')}`;
const loopId = (f) => path.basename(f, path.extname(f));
router.post('/save-loop', uploadLimiter, upload.single('file'), async (req, res, next) => {
    try {
        if (!req.file) {
            res.status(400).json({ status: 'error', message: 'No file.', timestamp: new Date().toISOString() });
            return;
        }
...
router.get('/loops/:id', async (req, res, next) => {
    try {
        ...
        if (!matched) {
            res.status(404).json({ status: 'error', message: `Loop not found: ${req.params.id}`, timestamp: new Date().toISOString() });
            return;
        }
        const buffer = await readFile(`${LOOPS_DIR}/${matched}`);
        if (!buffer) {
            res.status(404).json({ status: 'error', message: 'File missing.', timestamp: new Date().toISOString() });
            return;
        }
        const mimes = { '.wav': 'audio/wav', '.webm': 'audio/webm', '.ogg': 'audio/ogg', '.mp3': 'audio/mpeg' };
