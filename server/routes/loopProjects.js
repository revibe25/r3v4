...
const projectPath = (id) => `${PROJECTS_DIR}/${id}.r3project.json`;
router.post('/loopproject/save', async (req, res, next) => {
    try {
...
router.get('/loopproject/:id', async (req, res, next) => {
    try {
        if (!/^[0-9a-f-]{36}$/.test(req.params.id)) {
            res.status(400).json({ status: 'error', message: 'Invalid ID.', timestamp: new Date().toISOString() });
            return;
        }
        const p = await readJSON(projectPath(req.params.id));
        if (!p) {
            res.status(404).json({ status: 'error', message: `Not found: ${req.params.id}`, timestamp: new Date().toISOString() });
            return;
        }
...
router.get('/loopprojects', async (_req, res, next) => {
    try {
        ...
        const projects = (await Promise.all(files.map(async (f) => { const p = await readJSON(`${PROJECTS_DIR}/${f}`); return p ? { id: p.id, name: p.name, bpm: p.bpm, version: p.version, trackCount: p.tracks?.length ?? 0, updatedAt: p.updatedAt } : null; }))).filter(Boolean);
