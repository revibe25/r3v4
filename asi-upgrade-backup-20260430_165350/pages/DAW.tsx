...
import { MixSuggestionsPanel }   from '../components/mix-suggestions/MixSuggestionsPanel';
...
const Knob = memo(({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, userSelect: 'none', width: size }}>
      <div
        style={{
          position:     'relative',
          width:        size,
      ...
      >
        <div
          style={{
            ...
            inset:        3,
            borderRadius: '50%',
            background:   `conic-gradient(from ${-135}deg at 50% 50%, #222 0deg, #222 ${pct*270}deg, transparent ${pct*270}deg)`,
          }}
        />
        <div
          style={{
      ...
      <span style={{ fontSize: 9, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#5a5a5a' }}>{label}</span>
    </div>
  );
});
...
const VUMeter = memo(({ level, vertical = true, accent = '#a3e635', warn = '#f59e0b', clip = '#ef4444' }: {
  ...
  const bars = 12;
  return (
    <div style={{ display: 'flex', flexDirection: vertical ? 'column-reverse' : 'row', gap: 1, ...(vertical ? { height: 48 } : { width: 48 }) }}>
...
const Btn = memo(({
  children, onClick, active, danger, dim, style: sx, title,
}: {
  children: React.ReactNode; onClick?: () => void;
  active?: boolean; danger?: boolean; dim?: boolean;
  style?: React.CSSProperties; title?: string;
}) => {
  const bg    = active ? (danger ? 'rgba(220,38,38,0.2)' : 'rgba(245,158,11,0.2)')
              : dim    ? 'transparent'
              :          '#1a1a1a';
  const bdr   = active ? (danger ? 'rgba(220,38,38,0.6)' : 'rgba(245,158,11,0.6)')
              : dim    ? '#2a2a2a'
              :          '#333';
  const col   = active ? (danger ? '#f87171' : '#fbbf24')
              : dim    ? '#444'
              :          '#888';
  return (
    <button
      ...
      onMouseLeave={e => {
        if (!dim && !active) {
          ...
          (e.currentTarget as HTMLButtonElement).style.color       = col;
        }
      }}
    >
      {children}
...
const Led = memo(({ on, color = '#f59e0b', pulse }: { on: boolean; color?: string; pulse?: boolean }) => (
  <div
    style={{
      ...
      animation:   pulse && on ? 'daw-led-pulse 1s ease-in-out infinite' : 'none',
    }}
  />
));
Led.displayName = 'Led';
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2 bg-[#0d0d0d] border-b border-[#1c1c1c]"
      ...
      <div className="flex items-center gap-2 min-w-[140px]">
        <Led on={collabConnected} color="#00F5FF" pulse={collabConnected} />
        {editingName ? (
          <input
            autoFocus
        ...
        <div
          style={{
            ...
            height:       6,
            borderRadius: '50%',
            background:   syncColors[syncStatus] ?? '#444',
          }}
      ...
      <div className="flex items-center gap-1.5">
        ...
        <Btn onClick={engine.tapTempo} title="Tap Tempo (T)">TAP</Btn>
      </div>
        ...
        {sidebarTab === 'files' && (
          <>
            ...
            <div className="pt-2 border-t border-[#1c1c1c] mt-2">
              ...
              />
              <Btn onClick={() => uploadRef.current?.click()}>
                + UPLOAD
              </Btn>
            </div>
          </>
            ...
            {!collabEnabled ? (
              joining ? (
                <div className="space-y-2">
                  <input
                    ...
                    onKeyDown={e => {
                      if (e.key === 'Enter' && roomInput) {
                        ...
                        const colors = ['#f59e0b','#00F5FF','#22c55e','#a855f7','#ef4444'];
                  ...
                  />
                  <Btn onClick={() => setJoining(false)}>CANCEL</Btn>
                </div>
              ) : (
                <Btn onClick={() => setJoining(true)}>
                  JOIN ROOM
                </Btn>
              )
            ) : (
              <Btn danger onClick={collab.leaveRoom}>
                LEAVE ROOM
              </Btn>
            )}
    ...
    <div className="flex flex-col bg-[#0a0a0a] border-t border-[#1c1c1c]" style={{ height: 200 }}>
      ...
      <div className="flex items-center gap-3 px-3 py-1.5 border-b border-[#1c1c1c] flex-none">
        ...
        <div className="ml-auto flex gap-1">
          <Btn onClick={seq.clearPattern}>CLR</Btn>
          <Btn onClick={seq.duplicate}>DUP</Btn>
        ...
        {aiPanelTab === 'mix' && (
          <div className="p-3 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Btn onClick={triggerSuggestions}>ANALYSE</Btn>
            </div>

            {/* ── MVP Item 4: Mix Suggestion System ──────────────────────────
                MixSuggestionsPanel is wired to sessionMetrics.router.
                Three triggers: frequency masking · dynamic imbalance · arrangement conflict.
                PRD §8.4: panel exists; backend wiring complete via aiMix.router.
            ─────────────────────────────────────────────────────────────────── */}
            <MixSuggestionsPanel
              onSuggestionAccept={(decisionId: string) => {
                // Write accepted outcome to aiDecisionLog via tRPC
                // PRD §6: acceptance rate ≥40% — every outcome must be logged
                const token   = localStorage.getItem('r3_token');
                const apiBase = (import.meta.env?.VITE_API_URL as string | undefined) ?? '';
                fetch(`${apiBase}/trpc/aiMix.submitSuggestionOutcome`, {
                  ...
                  body: JSON.stringify({
                    json: { decisionId, outcome: 'accepted' as const },
                  }),
                }).catch(() => { /* non-fatal — aiDecisionLog write failure does not block UI */ });
              }}
              onSuggestionReject={(decisionId: string) => {
                const token   = localStorage.getItem('r3_token');
                const apiBase = (import.meta.env?.VITE_API_URL as string | undefined) ?? '';
                fetch(`${apiBase}/trpc/aiMix.submitSuggestionOutcome`, {
                  method:  'POST',
                  ...
                  body: JSON.stringify({
                    json: { decisionId, outcome: 'rejected' as const },
                  }),
                }).catch(() => { /* non-fatal */ });
              }}
            />
            ...
            {aiThinking && (
              <div className="flex items-center gap-2 py-2">
                <div className="flex gap-0.5">
                  {[0,1,2].map(i => (
                    <div key={i} style={{ width: 4, height: 4, borderRadius: '50%', background: '#f59e0b', animation: `bounce 1s infinite ${i * 0.15}s` }}
                         />
                  ))}
                </div>
            ...
            <div className="flex-1 p-3 space-y-2 overflow-y-auto" style={{ maxHeight: 320 }}>
              ...
              {aiThinking && aiPanelTab === 'coproducer' && (
                <div className="flex gap-1 pl-1">
                  {[0,1,2].map(i => (
                    <div key={i} style={{ width: 6, height: 6, borderRadius: '50%', background: '#444', animation: `bounce 1s infinite ${i * 0.2}s` }}
                         />
                  ))}
                </div>
              )}
...
const AISuggestionCard = memo(({
  ...
  const TYPE_COLORS: Record<string, string> = {
    mix: '#00F5FF', arrangement: '#f59e0b', mastering: '#22c55e',
      ...
      <div className="flex items-center justify-between">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          <div
            ...
            aria-valuemin={0}
            aria-valuemax={100}
            style={{ width: 48, height: 2, background: '#222' }}
          >
            <div style={{ height: '100%', width: `${suggestion.confidence * 100}%`, background: color }} />
          </div>
          <span style={{ fontSize: 8, fontFamily: '"JetBrains Mono",monospace', color }}>
...
export default function DAW() {
  ...
  return (
    <>
      <style>{`
        @keyframes daw-led-pulse { 0%,100%{opacity:1} 50%{opacity:0.4} }
      ...
      <div style={{
        ...
        <span style={{ fontSize: 8, color: '#333', letterSpacing: '0.1em', marginRight: 4 }}>VIEW</span>
        <Btn
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
          <ThemeSwitcher />
          <SessionChip />
          <span style={{ fontSize: 8, color: '#333', letterSpacing: '0.1em' }}>ZOOM</span>
          <Btn onClick={() => useDAWStore.getState().setZoom(useDAWStore.getState().zoom * 0.8)}>−</Btn>
          <span style={{ fontSize: 9, fontFamily: '"JetBrains Mono",monospace', color: '#555', width: 32, textAlign: 'center' }}>
            {useDAWStore(s => s.zoom).toFixed(1)}×
          </span>
          <Btn onClick={() => useDAWStore.getState().setZoom(useDAWStore.getState().zoom * 1.2)}>+</Btn>

          <div style={{ width: 1, height: 16, background: '#2a2a2a', margin: '0 4px' }} />

          <span style={{ fontSize: 8, color: '#333', letterSpacing: '0.1em' }}>ROWS</span>
...
const StatusBar = memo(() => {
  const { playing, recording, collabConnected, collabUsers, bpm, timeSignature } = useDAWStore();
  return (
    <>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        <Led on={playing}   color="#22c55e" />
        <Led on={recording} color="#ef4444" pulse={recording} />
        <span style={{ fontSize: 8, color: '#333', fontFamily: '"JetBrains Mono",monospace', letterSpacing: '0.1em' }}>
          {recording ? 'REC' : playing ? 'PLAY' : 'STOPPED'}
        </span>
      </div>
      <div style={{ width: 1, height: 12, background: '#2a2a2a' }} />
      <span style={{ fontSize: 8, fontFamily: '"JetBrains Mono",monospace', color: '#333' }}>
      ...
      {collabConnected && (
        <>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <Led on color="#00F5FF" />
            <span style={{ fontSize: 8, color: '#00F5FF', fontFamily: '"JetBrains Mono",monospace' }}>
              {collabUsers.length + 1} IN SESSION
            </span>
          </div>
        </>
      )}
      <div style={{ marginLeft: 'auto', fontSize: 8, color: '#252525', fontFamily: '"JetBrains Mono",monospace', letterSpacing: '0.05em' }}>
        R3 v4.1 · SPACE=play · R=rec · T=tap · M=midi · A=ai · ±=zoom
      </div>
    </>
  );
});
