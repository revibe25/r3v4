...
## Integration with LLPTE

DJ crossfader transitions feed directly into the LLPTE `transitionGraph` node.
When `sessionId` is active, transition events are scored via Camelot wheel harmonic
analysis and logged to `aiDecisionLog`. The AI can suggest or auto-apply transition
types (Key-Match Crossfade, Filter Sweep, Echo Freeze etc.) based on confidence gates.
See AI_MIXING.md for full transition type documentation.
...
if (latency > 5) {
  // logger.warn('High latency — consider reducing buffer size')
}
```
...
tempo.subscribe((state) => {
  // logger.info(`BPM: ${state.bpm}, Pitch: ${state.pitchShift}`)
});
```
...
tempo.subscribe((state) => {
  // logger.info(`Current BPM: ${state.bpm}`)
});
...
// logger.info(`Jump to ${position}s`)
...
// logger.info(`${activeCues.length} cues set`)
```
...
beatSync.subscribe((grid) => {
  if (grid) {
    // logger.info(`Beat grid: ${grid.markers.length} beats`)
  }
});
```
...
if (crossfader.getLatency() > 5) {
  // logger.warn('High crossfader latency')
