# R3VIBE Native - Audio Architecture
...
| Audio Driver | 1-2ms | ASIO/Core Audio/ALSA |
...
**Windows (ASIO)**
- Use ASIO drivers (lower latency than WDM)
- Install LoopMIDI for virtual routing
- Disable exclusive mode for system sounds

**macOS (Core Audio)**
- Core Audio is optimized by default
- Use AudioUnit for low latency
- 512 sample buffer typical

**Linux (ALSA/PulseAudio)**
- ALSA offers lowest latency
- PulseAudio adds ~10ms overhead
- Jack Audio Server recommended for live use
...
private measureCrossfaderLatency() {
  ...
  if (latency > 10) {
    console.warn('High crossfader latency detected');
  }
}
```
...
console.log(`CPU Load: ${cpuLoad.toFixed(1)}%`);
```
...
measures.forEach(m => console.log(`Processing time: ${m.duration.toFixed(2)}ms`));
```
...
// Monitor and warn
if (cpuLoad > 70) {
  console.warn('High CPU load - reducing effects');
...
drumTrack.effectChain.byppassEffect('reverb');
```
