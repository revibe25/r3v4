...
export class VocalSpectra /* implements DSPPlugin */ {
    ...
    prepare(sampleRate, maxBlockSize, layout) {
        ...
        for (const p of VocalSpectraParameters) {
            ...
            if (p.smoothingTimeMs > 0) {
                this.paramSmoothers[p.id] = new Smoother(this.sampleRate, p.smoothingTimeMs, p.defaultValue);
            }
        }
    }
    processBlock(inputs, outputs, parameters) {
        ...
        let signal = this.pitch.correct(input, pitchData, this.paramSmoothers['correction']?.value ?? parameters['correction']?.[0] ?? 100);
