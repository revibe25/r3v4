...
import { useSessionMetricsStore, selectActions, selectSessionId, selectIsSessionActive, } from '../stores/session-metrics.store';
export function useSessionLifecycle({ isPlaying, }) {
    ...
    // ── Transport start ────────────────────────────────────────────────────────
    useEffect(() => {
        if (!isPlaying || isSessionActive || startInProgress.current)
            return;
        startInProgress.current = true;
        const newSessionId = uuidv4();
        sessionStarted(newSessionId);
        startMutation.mutate({ sessionId: newSessionId }, {
    ...
    useEffect(() => {
        if (isPlaying || !isSessionActive || endInProgress.current)
            return;
        const currentSessionId = sessionIdRef.current;
        if (!currentSessionId)
            return;
        endInProgress.current = true;
        endMutation.mutate({ sessionId: currentSessionId }, { onSettled: () => { endInProgress.current = false; } });
    }, [isPlaying, isSessionActive, endMutation]);
    // ── logAction — fire-and-forget ────────────────────────────────────────────
    const logAction = useCallback((delta) => {
