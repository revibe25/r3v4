...
export function pixelsToTime(pixels, zoom) {
    if (!Number.isFinite(zoom) || zoom <= 0) {
        throw new RangeError(`pixelsToTime: zoom must be a finite positive number, got ${zoom}`);
    }
    return pixels / zoom;
}
/**
 ...
 */
export function timeToPixels(time, zoom) {
    if (!Number.isFinite(zoom) || zoom <= 0) {
        throw new RangeError(`timeToPixels: zoom must be a finite positive number, got ${zoom}`);
    }
    return time * zoom;
}
/**
...
/**
 * Default arrangement timeline zoom: pixels rendered per second of audio.
 * Used as the baseline zoom level in timeline components.
 * Imported by components/instruments/timeline.tsx.
 */
