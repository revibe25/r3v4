 ...
 */
export function panToGains(pan) {
 ...
 */
export function secondsToBarsBeatsTicks(seconds, bpm, timeSignature) {
 ...
 */
export function barsBeatTicksToSeconds(bars, beats, ticks, bpm, timeSignature) {
    const beatsPerBar = timeSignature.numerator;
 ...
 */
export function createSilentBuffer(audioContext, duration, channels = 2) {
 ...
 */
export function mixBuffers(buffer1, buffer2, ratio = 0.5) {
 ...
 */
export function normalizeBuffer(buffer, targetLevel = 0.95) {
 ...
 */
export function cloneEffectState(state) {
    return {
        ...state,
 ...
 */
export function areEffectStatesEqual(state1, state2) {
    ...
    if (state1.enabled !== state2.enabled)
        return false;
    if (state1.bypassed !== state2.bypassed)
        return false;
    if (state1.wet !== state2.wet)
        return false;
    // Compare parameters
 ...
 */
export function cloneChannelState(state) {
    return {
        ...state,
    };
}
 ...
 */
export function validateChannelState(state) {
 ...
 */
export function estimateProjectSize(state) {
 ...
 */
export function calculatePeak(data) {
    let peak = 0;
    for (let i = 0; i < data.length; i++) {
        peak = Math.max(peak, Math.abs(data[i]));
    }
 ...
 */
export function smoothValue(currentValue, targetValue, smoothingFactor = 0.3) {
