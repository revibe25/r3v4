 ...
 */
export function serializeProjectToBlob(state) {
 ...
 */
function validateProjectStructure(state) {
 ...
 */
export function createProjectBackup(state) {
    return JSON.parse(JSON.stringify(state));
}
/**
 * Compare two project states
 */
export function areProjectsEqual(state1, state2) {
    // Quick checks
    if (state1.timestamp !== state2.timestamp)
        return false;
    if (state1.channels.length !== state2.channels.length)
        return false;
    if (state1.effectChains.length !== state2.effectChains.length)
        return false;
 ...
 */
export function getProjectSummary(state) {
