...
function ReactiveIcosphere({ fftRef, beatFlashRef, colorBase, colorAccent }) {
    ...
    return (_jsxs("mesh", { ref: meshRef, castShadow: true, children: [_jsx("icosahedronGeometry", { args: [1.5, 5] }), _jsx("shaderMaterial", { vertexShader: VERT, fragmentShader: FRAG, uniforms: uniforms, transparent: true, side: THREE.DoubleSide, depthWrite: false })] }));
}
// ── Particle ring ─────────────────────────────────────────────────────────────
function ParticleRing({ fftRef, count = 128, radius = 3.2, color = "#00aaff" }) {
    ...
    useFrame(({ clock }) => {
        ...
        for (let i = 0; i < count; i++) {
            ...
            pos.setXYZ(i, Math.cos(a + t * 0.1) * r, Math.sin(t * 0.3 + i * 0.05) * (0.2 + e * 0.5), Math.sin(a + t * 0.1) * r);
        }
        pos.needsUpdate = true;
    });
    return _jsx("points", { ref: ptsRef, geometry: geo, children: _jsx("pointsMaterial", { color: color, size: 0.06, sizeAttenuation: true, transparent: true, opacity: 0.8 }) });
}
// ── Camera controller ─────────────────────────────────────────────────────────
...
export function AudioReactiveScene({ colorBase = "#1a0066", colorAccent = "#00ff88", showRing = true, ringColor = "#00aaff", animateCamera = true, }) {
    ...
    return (_jsxs(_Fragment, { children: [_jsx("ambientLight", { intensity: 0.15 }), _jsx("pointLight", { position: [0, 3, 3], intensity: 1.2, color: colorAccent, castShadow: true }), _jsx("pointLight", { position: [0, -3, -3], intensity: 0.6, color: colorBase }), animateCamera && _jsx(AudioReactiveCamera, { fftRef: fftRef }), _jsx(ReactiveIcosphere, { fftRef: fftRef, beatFlashRef: beatFlashRef, colorBase: colorBase, colorAccent: colorAccent }), showRing && _jsx(ParticleRing, { fftRef: fftRef, color: ringColor }), _jsx("fog", { attach: "fog", args: ["#000000", 8, 20] })] }));
}
