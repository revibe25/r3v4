...
export function AudioVisualizer({ getAnalyserData: externalGetAnalyserData, isInitialized: externalIsInitialized, isActive = true, width: propWidth, height: propHeight, className = '', showControls = true, onVisualizationModeChange, } = {}) {
    ...
    useEffect(() => {
        ...
        matrixRef.current = Array.from({ length: Math.floor(w / 14) }, (_, i) => new MatrixDrop(i * 14, h));
    ...
    const drawBars = useCallback((ctx, data, w, h) => {
        const count = 64;
        const gap = 2;
    ...
    const drawWave = useCallback((ctx, data, w, h) => {
    ...
    const drawCircular = useCallback((ctx, data, w, h) => {
        const cx = w / 2;
        const cy = h / 2;
    ...
    const drawParticles = useCallback((ctx, data, w, h) => {
        const avg = data.reduce((a, b) => a + b, 0) / data.length / 255;
    ...
    const drawOscilloscope = useCallback((ctx, data, w, h) => {
        const waveData = getWaveData() || data;
        const cy = h / 2;
        // Grid
    ...
    const drawSpectrogram = useCallback((ctx, data, w, h) => {
        const col = [];
        ...
        while (spectroRef.current.length > maxCols)
            spectroRef.current.shift();
    ...
    const drawTerrain = useCallback((ctx, data, w, h) => {
        const slices = 24;
        const points = 80;
    ...
    const drawGalaxy = useCallback((ctx, data, w, h) => {
        const cx = w / 2;
        const cy = h / 2;
        const avg = data.reduce((a, b) => a + b, 0) / data.length / 255;
    ...
    const drawDNA = useCallback((ctx, data, w, h) => {
    ...
    const drawFlame = useCallback((ctx, data, w, h) => {
        const cols = 64;
        const colW = w / cols;
    ...
    const drawMatrix = useCallback((ctx, data, w, h) => {
        const avg = data.reduce((a, b) => a + b, 0) / data.length / 255;
    ...
    const drawAurora = useCallback((ctx, data, w, h) => {
        const t = timeRef.current * 0.001;
        const layers = 5;
    ...
    useEffect(() => {
        ...
        const animate = (timestamp) => {
            ...
            if (data) {
                ...
                ctx.fillText(isEffectivelyListening ? "Waiting for audio..." : "Click Start to begin", w / 2, h / 2);
                // Idle pulse
    ...
    return (_jsxs("div", { className: className, style: {
            ...
            transition: "height 0.3s ease",
                    ...
                    cursor: "pointer",
                    userSelect: "none",
                }, onClick: () => setCollapsed(prev => !prev), children: [_jsxs("div", { style: { display: "flex", alignItems: "center", gap: 8 }, children: [_jsx("div", { style: {
                                    width: 14,
                                    height: 14,
                                    display: "flex",
                                    alignItems: "center",
                                    ...
                                    color: BLACK,
                                    fontWeight: 900,
                                }, children: "\u25C6" }), _jsxs("div", { children: [_jsx("span", { style: { fontSize: 11, fontWeight: 700, letterSpacing: 2, lineHeight: 1, color: "var(--white)", textTransform: "uppercase" }, children: "VISUALIZER" }), _jsx("div", { style: { fontSize: 8, color: "var(--dj-dim)", marginTop: 2, letterSpacing: 1 }, children: collapsed ? `${MODES.find(m => m.id === mode)?.label.toUpperCase()} · ${isEffectivelyListening ? "ACTIVE" : "IDLE"}` : `${fps} FPS · ${MODES.find(m => m.id === mode)?.label.toUpperCase()}` })] })] }), _jsxs("div", { style: { display: "flex", alignItems: "center", gap: 8 }, onClick: (e) => e.stopPropagation(), children: [_jsxs("div", { style: { display: "flex", alignItems: "center", gap: 4 }, children: [_jsx("span", { style: { fontSize: 8, color: "var(--dj-dim)", letterSpacing: 1 }, children: "VOL" }), _jsx("div", { style: { display: "flex", gap: 1, alignItems: "center" }, children: Array.from({ length: 16 }).map((_, i) => {
                                        ...
                                        }) }), !collapsed && (_jsxs("span", { style: { fontSize: 8, color: "var(--dj-dim)", width: 26 }, children: [volumePercent, "%"] }))] }), !collapsed && (_jsxs("div", { style: { display: "flex", alignItems: "center", gap: 4 }, children: [_jsx("span", { style: { fontSize: 8, color: "var(--dj-dim)", letterSpacing: 1 }, children: "PK" }), _jsx("div", { style: { display: "flex", gap: 1, alignItems: "center" }, children: Array.from({ length: 8 }).map((_, i) => {
                                        ...
                                        }) })] })), hasOwnAudio && (_jsx("button", { onClick: isListening ? stopAudio : startAudio, style: {
                                ...
                                }, children: isListening ? "■ STOP" : "▶ START" })), !collapsed && (_jsx("button", { onClick: () => setShowUI(false), style: {
                                    ...
                                    padding: "2px 4px",
                                    fontFamily: "inherit",
                                }, title: "Hide UI (press H to toggle)", children: "\u2715" }))] })] }), !collapsed && (_jsxs(_Fragment, { children: [_jsx("canvas", { ref: canvasRef, style: {
                            flex: 1,
                            width: "100%",
                            display: "block",
                            cursor: "crosshair",
                        }, onClick: () => !showUI && setShowUI(true) }), showUI && (_jsxs("div", { style: {
                            padding: "8px 12px",
                            background: SURFACE,
                            ...
                            flexShrink: 0,
                                    ...
                                    flexWrap: "wrap",
                                    justifyContent: "center",
                                }, children: MODES.map((m) => (_jsxs("button", { onClick: () => {
                                        setMode(m.id);
                                        spectroRef.current = [];
                                    ...
                                    }, children: [_jsx("span", { style: { fontSize: 10 }, children: m.icon }), m.label] }, m.id))) }), _jsxs("div", { style: {
                                    display: "flex",
                                    alignItems: "center",
                                    justifyContent: "center",
                                    gap: 16,
                                    flexWrap: "wrap",
                                }, children: [_jsxs("div", { style: { display: "flex", alignItems: "center", gap: 4 }, children: [_jsx("span", { style: { fontSize: 8, color: "var(--dj-dim)", letterSpacing: 1, marginRight: 2 }, children: "THEME" }), Object.entries(THEMES).map(([key, t]) => (_jsx("button", { onClick: () => setTheme(key), style: {
                                                    width: 14,
                                                    height: 14,
                                                    borderRadius: 0,
                                                    ...
                                                    cursor: "pointer",
                                                    padding: 0,
                                                }, title: t.name }, key)))] }), _jsxs("div", { style: { display: "flex", alignItems: "center", gap: 6 }, children: [_jsx("span", { style: { fontSize: 8, color: "var(--dj-dim)", letterSpacing: 1 }, children: "GAIN" }), _jsxs("div", { style: { position: "relative", width: 80, height: 14, display: "flex", alignItems: "center" }, children: [_jsx("div", { style: { position: "absolute", left: 0, right: 0, height: 2, background: BORDER } }), _jsx("div", { style: { position: "absolute", left: 0, width: `${((sensitivity - 0.5) / 2.5) * 100}%`, height: 2, background: ACID } }), _jsx("input", { type: "range", min: "0.5", max: "3", step: "0.1", value: sensitivity, onChange: (e) => setSensitivity(parseFloat(e.target.value)), style: {
                                                            position: "absolute",
                                                            left: 0,
                                                            right: 0,
                                                            width: "100%",
                                                            ...
                                                            background: ACID,
                                                            pointerEvents: "none",
                                                        } })] }), _jsxs("span", { style: { fontSize: 8, color: "#555", width: 28 }, children: [sensitivity.toFixed(1), "x"] })] })] })] })), error && (_jsxs("div", { style: {
                            position: "absolute",
                            top: "50%",
                            left: "50%",
                            ...
                            textAlign: "center",
                            zIndex: 20,
                        }, children: [_jsx("p", { style: { color: "#ff4400", fontSize: 12, margin: 0, letterSpacing: 1, textTransform: "uppercase" }, children: error }), _jsx("button", { onClick: startAudio, style: {
                                    marginTop: 12,
                                    background: ACID,
                                    border: "none",
                                    color: BLACK,
                                    ...
                                    letterSpacing: 2,
                                    textTransform: "uppercase",
                                }, children: "RETRY" })] })), !showUI && !collapsed && (_jsx("div", { style: {
                            position: "absolute",
                            bottom: 10,
                            left: "50%",
                            ...
                            letterSpacing: 1,
                            textTransform: "uppercase",
                        }, children: "CLICK TO SHOW CONTROLS" }))] }))] }));
}
