...
export const AdvancedMeter = ({ level, peak, width = 20, height = 100, orientation = 'vertical', showPeak = true, showDb = false, }) => {
    ...
    return (_jsxs("div", { className: "relative", children: [_jsx("canvas", { ref: canvasRef, className: "rounded", style: { display: 'block' } }), showDb && (_jsxs("div", { className: "absolute top-0 right-full mr-1 text-xs text-muted-foreground font-mono", children: [(20 * Math.log10(Math.max(level, 0.001))).toFixed(1), " dB"] }))] }));
};
