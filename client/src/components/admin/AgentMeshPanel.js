...
const AGENT_KEYS = ["spectral", "auth"];
// ─── Helpers ──────────────────────────────────────────────────────────────────
function nowTs() {
    const n = new Date();
...
function ConfBar({ value }) {
    ...
    return (_jsxs("div", { style: { display: "flex", alignItems: "center", gap: 8 }, children: [_jsx("span", { style: { fontSize: 11, color: T.z400, minWidth: 70, fontFamily: "'JetBrains Mono', monospace" }, children: "confidence" }), _jsx("div", { style: { flex: 1, height: 6, background: T.z800, borderRadius: 3, overflow: "hidden" }, children: _jsx("div", { style: {
...
function SLAChip({ label, value, status }) {
    ...
    return (_jsxs("div", { style: {
        ...
        }, children: [_jsx("div", { style: { fontSize: 11, color: T.z400, fontFamily: "'JetBrains Mono', monospace" }, children: label }), _jsx("div", { style: { fontSize: 13, fontWeight: 500, color: statusColor, fontFamily: "'JetBrains Mono', monospace" }, children: value })] }));
}
// ─── Agent Card ───────────────────────────────────────────────────────────────
function AgentCard({ agentKey, state, onRun, loading, }) {
    const manifest = MANIFESTS[agentKey];
    const [input, setInput] = useState("");
    const lat = state.latency;
    const latStatus = lat === null ? "neutral" : lat <= manifest.latencySLA ? "ok" : lat <= 25 ? "warn" : "fail";
    const conf = state.confidence;
    const confStatus = conf === null ? "neutral" : conf >= manifest.confidenceGate ? "ok" : conf >= 0.5 ? "warn" : "fail";
    return (_jsxs("div", { style: {
            background: T.z900,
            ...
            flexDirection: "column",
                ...
                }, children: [_jsxs("div", { style: { display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }, children: [_jsx("div", { style: {
                                    fontSize: 12, fontWeight: 500,
                                    fontFamily: "'JetBrains Mono', monospace",
                                    color: manifest.color,
                                }, children: manifest.id }), _jsx("div", { style: {
                                    fontSize: 11, padding: "2px 8px", borderRadius: 20,
                                    background: `${manifest.tagColor}18`,
                                    border: `1px solid ${manifest.tagColor}40`,
                                    color: manifest.tagColor, fontWeight: 500,
                                }, children: manifest.tag })] }), _jsx("div", { style: { fontSize: 14, fontWeight: 500, color: T.z100, marginBottom: 6 }, children: agentKey === "spectral" ? "Spectral Analyzer" : "Auth & Routing" }), _jsxs("div", { style: { display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 4 }, children: [_jsx("div", { style: {
                                    fontSize: 11, padding: "2px 8px", borderRadius: 20,
                                    background: T.z800, border: `1px solid ${T.z700}`,
                                    color: T.z400,
                                }, children: manifest.tools.join(" · ") }), _jsxs("div", { style: {
                                    fontSize: 11, padding: "2px 8px", borderRadius: 20,
                                    background: `${T.amber}18`, border: `1px solid ${T.amber}40`,
                                    color: T.amber, fontWeight: 500,
                                }, children: ["gate: ", manifest.confidenceGate] })] }), _jsxs("div", { style: {
                            fontSize: 11, color: T.z600,
                            fontFamily: "'JetBrains Mono', monospace",
                        }, children: ["scope: ", manifest.scope] })] }), _jsxs("div", { style: { padding: "12px 14px", flex: 1, display: "flex", flexDirection: "column", gap: 10 }, children: [_jsxs("div", { style: { display: "flex", gap: 8 }, children: [_jsx(SLAChip, { label: "latency SLA", value: lat !== null ? `${lat}ms` : "— ms", status: latStatus }), _jsx(SLAChip, { label: "confidence", value: conf !== null ? conf.toFixed(2) : "—", status: confStatus }), _jsx(SLAChip, { label: "last action", value: state.action, status: state.action === "done" ? "ok" : state.action === "error" ? "fail" : "neutral" })] }), _jsx(ConfBar, { value: conf }), _jsxs("div", { style: { display: "flex", flexDirection: "column", gap: 6 }, children: [_jsx("div", { style: {
                                    fontSize: 11, color: T.z400, fontWeight: 500,
                                    textTransform: "uppercase", letterSpacing: "0.04em",
                                    fontFamily: "'JetBrains Mono', monospace",
                                }, children: "send task to agent" }), _jsx("textarea", { value: input, onChange: e => setInput(e.target.value), placeholder: manifest.placeholder, style: {
                                    ...
                                    outline: "none",
                                } }), _jsx("button", { onClick: () => { if (input.trim()) {
                                    onRun(agentKey, input.trim());
                                    setInput("");
                                ...
                                }, children: loading ? "Running…" : "Run ↗" })] }), _jsxs("div", { style: { display: "flex", flexDirection: "column", gap: 4 }, children: [_jsx("div", { style: {
                                    fontSize: 11, color: T.z400, fontWeight: 500,
                                    textTransform: "uppercase", letterSpacing: "0.04em",
                                    fontFamily: "'JetBrains Mono', monospace",
                                }, children: "agent output" }), _jsx("div", { style: {
                                ...
                                }, children: state.outputLines.length === 0
                                    ? _jsx("span", { style: { color: T.z600 }, children: "awaiting task\u2026" })
                                    : state.outputLines.map((line, i) => {
                                        const lineColor = line.cls === "ok" ? T.emerald :
                                            line.cls === "warn" ? T.amber :
                                                line.cls === "err" ? T.red :
                                                    line.cls === "bus" ? T.cyan :
                                                        line.cls === "sys" ? T.z600 : T.z400;
                                        return (_jsx("span", { style: { display: "block", color: lineColor }, children: line.text }, i));
                                    }) })] })] })] }));
}
// ─── Bus Entry Row ────────────────────────────────────────────────────────────
function BusRow({ entry }) {
    ...
    return (_jsxs("div", { style: { display: "flex", gap: 8, alignItems: "flex-start" }, children: [_jsx("span", { style: { color: T.z600, flexShrink: 0, minWidth: 52, fontFamily: "'JetBrains Mono', monospace", fontSize: 11 }, children: entry.ts }), _jsx("span", { style: { color: dirColor, flexShrink: 0, fontWeight: 500, fontFamily: "'JetBrains Mono', monospace", fontSize: 11 }, children: dirLabel }), _jsxs("span", { style: { color: T.z400, fontFamily: "'JetBrains Mono', monospace", fontSize: 11 }, children: [entry.from, " \u2192 ", entry.to, " \u00B7 ", entry.text] })] }));
}
// ─── Agent Mesh Panel ─────────────────────────────────────────────────────────
export function AgentMeshPanel() {
    ...
    const busEmit = useCallback((from, to, type, text) => {
        setBusFiring(true);
        setTimeout(() => setBusFiring(false), 400);
        setBusLog(prev => [...prev, { ts: nowTs(), from, to, type, text }]);
    }, []);
    const runAgent = useCallback(async (agentKey, userMsg) => {
        const manifest = MANIFESTS[agentKey];
    ...
    return (_jsxs("div", { style: {
      ...
      ` }), _jsxs("div", { style: {
                    ...
                    borderBottom: `1px solid ${T.z800}`,
                        ...
                        } }), _jsx("h1", { style: { fontSize: 15, fontWeight: 500, letterSpacing: "-0.01em" }, children: "R3 v4 \u2014 agent mesh" }), _jsx("div", { style: { flex: 1 } }), _jsx("div", { style: {
                            fontSize: 11, padding: "2px 8px", borderRadius: 20,
                            background: T.z800, border: `1px solid ${T.z700}`,
                            color: T.z400,
                        }, children: "BroadcastChannel \u00B7 r3v4-agent-bus" }), _jsx("div", { style: {
                            fontSize: 11, padding: "2px 8px", borderRadius: 20,
                            background: `${T.emerald}18`, border: `1px solid ${T.emerald}40`,
                            color: T.emerald, fontWeight: 500,
                        }, children: "dry-run default" })] }), _jsx("div", { style: {
                    display: "grid",
                    gridTemplateColumns: "1fr 1fr",
                    gap: 12,
                    padding: "12px 16px",
                }, children: AGENT_KEYS.map(key => (_jsx(AgentCard, { agentKey: key, state: agentStates[key], onRun: (k, t) => void runAgent(k, t), loading: loadingKey === key }, key))) }), showDryBanner && (_jsxs("div", { style: {
                    margin: "0 16px 8px",
                    padding: "8px 12px",
                    borderRadius: 6,
                    ...
                    fontSize: 12, color: T.amber,
                }, children: ["\u26A0 Patch proposed. ", _jsx("strong", { children: "dry-run mode" }), " \u2014 no files written. A .bak backup would precede any apply. Run ", _jsx("code", { style: { fontFamily: "'JetBrains Mono', monospace" }, children: "pnpm tsc --noEmit" }), " to verify before applying."] })), _jsxs("div", { style: { padding: "0 16px" }, children: [_jsxs("div", { style: {
                            display: "flex", alignItems: "center", gap: 8,
                            padding: "10px 0 6px",
                            borderTop: `1px solid ${T.z800}`,
                        }, children: [_jsx("span", { style: { fontSize: 12, fontWeight: 500, color: T.z400, fontFamily: "'JetBrains Mono', monospace" }, children: "agent bus log" }), _jsx("div", { style: { flex: 1 } }), _jsx("button", { onClick: () => setBusLog([{ ts: nowTs(), from: "system", to: "*", type: "broadcast", text: "bus log cleared" }]), style: {
                        ...
                        }, children: busLog.map((entry, i) => _jsx(BusRow, { entry: entry }, i)) })] })] }));
}
