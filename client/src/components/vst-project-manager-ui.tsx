...
export function VSTProjectManagerUI({ onSave, onLoad }: ProjectManagerUIProps) {
  ...
  return (
    <div className="w-full bg-[var(--void)] text-[var(--daw-fg)] font-mono space-y-6">
      {status&&(
      ...
      <div className="space-y-3">
        <div>
          <label className="block text-[10px] tracking-widest uppercase text-[var(--daw-fg)] mb-1.5">Project Name</label>
          <input value={projectName} onChange={e=>setProjectName(e.target.value)} placeholder="untitled-project" disabled={loading}
            className="w-full bg-[#0d0d0d] border border-[#2a2a2a] text-[var(--daw-fg)] font-mono text-xs tracking-wider px-3 py-2 focus:outline-none focus:border-[#a3e635] placeholder:text-[var(--dj-dimmer)] disabled:opacity-40 rounded-none" />
        </div>
      ...
      <div className="border-t border-[var(--t-b2x)]" />
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <span className="text-[10px] tracking-widest uppercase text-[var(--daw-fg)]">Backups <span className="text-[#555]">({backups.length}/10)</span></span>
          <button onClick={handleBackup} disabled={loading} className={G}><Download className="h-3 w-3" /> Create Backup</button>
        </div>
        {backups.length===0?(
          <div className="border border-dashed border-[var(--t-b2x)] py-8 text-center">
            <FileJson className="h-6 w-6 mx-auto mb-2 text-[var(--dj-dimmer)]" />
            <p className="text-[10px] tracking-widest uppercase text-[var(--daw-fg)]">No backups yet</p>
...
function BackupRow({ backup, onRestore, disabled }: { backup:{name:string;timestamp:number;data:SerializedVSTChain}; onRestore:()=>void; disabled:boolean }) {
  ...
  return (
    <div className="border border-[var(--t-b2x)] px-4 py-3 flex items-center justify-between gap-4 hover:border-[#2a2a2a] transition-colors">
      <div className="min-w-0 space-y-1">
        <div className="text-xs tracking-wider text-[var(--daw-fg)] truncate">{backup.name}</div>
        ...
        <div className="flex gap-3 text-[10px] tracking-widest uppercase">
          <span className="text-[var(--daw-fg)]">{chainCount} chain{chainCount!==1?'s':''}</span>
          <span className="text-[var(--dj-dimmer)]">·</span>
          <span className="text-[var(--daw-fg)]">{fxCount} effect{fxCount!==1?'s':''}</span>
        </div>
      </div>
