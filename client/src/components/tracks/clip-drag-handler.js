...
export const useClipDrag = (clipId, initialX, initialTime, snapEnabled, onPositionChange, onDragEnd) => {
    useEffect(() => {
        if (!clipId)
            return;
...
// Clip resize hook
export const useClipResize = (clipId, initialX, initialDuration, snapEnabled, onDurationChange, onResizeEnd) => {
    useEffect(() => {
        if (!clipId)
            return;
        const handleMouseMove = (e) => {
