...
export function useMixSuggestions() {
    ...
    const [latencyMs, setLatencyMs] = useState(0); // ← ADD THIS LINE
    ...
    const mutation = trpc.daw["ai.suggestions"].useMutation({
        ...
        onSuccess: async (data) => {
            ...
            const ids = await Promise.all(newSuggestions.map((s) => recordDecisionMutRef.current
                .mutateAsync({
                sessionId,
                nodeId: "aiMixEngine",
    ...
    const analyse = useCallback((tracks, bpm, position = 0) => {
        mutation.mutate({
            tracks: tracks.map(t => ({
                type: 'audio',
                color: '#a3e635',
    ...
    const accept = useCallback((idx) => {
        ...
        if (id) {
            recordOutcomeMutRef.current.mutate({ id, outcome: "accepted" }, { onError: (err) => console.error("[useMixSuggestions] accept update failed:", err)
            });
        }
    }, []);
    const reject = useCallback((idx) => {
        setRejectedIds(prev => new Set(prev).add(idx));
        setAcceptedIds(prev => { const s = new Set(prev); s.delete(idx); return s; });
        const id = decisionIdsRef.current[idx];
        if (id) {
            recordOutcomeMutRef.current.mutate({ id, outcome: "rejected" }, { onError: (err) => console.error("[useMixSuggestions] reject update failed:", err)
            });
        }
    }, []);
