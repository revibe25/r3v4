...
export function useMidiSequencer() {
    ...
    useEffect(() => {
        ...
        navigator.requestMIDIAccess({ sysex: false }).then(access => {
            ...
            if (outputs.length > 0)
                midiOutRef.current = outputs[0];
            access.onstatechange = () => {
                const updated = Array.from(access.outputs.values());
                midiOutRef.current = (updated[0] ?? null);
            };
        }).catch(() => { });
    }, []);
    ...
    useEffect(() => {
        const rebuild = () => {
            ...
            for (const note of pattern.notes) {
                if (!stepMap.has(note.step))
                    stepMap.set(note.step, []);
                stepMap.get(note.step).push(note);
            }
            const steps = Array.from({ length: pattern.steps }, (_, i) => stepMap.get(i) ?? []);
            const seq = new Tone.Sequence((time, notesAtStep) => {
                const step = seq._index ?? 0;
                useDAWStore.getState().setSequencerStep(step);
                if (!Array.isArray(notesAtStep))
                    return;
        ...
        rebuild();
        const unsub = useDAWStore.subscribe(s => [s.activePatternId, s.midiPatterns], rebuild);
        return () => {
            unsub();
            sequenceRef.current?.dispose();
        };
    ...
    useEffect(() => {
        const unsub = useDAWStore.subscribe(s => s.playing, playing => {
            if (!sequenceRef.current)
                return;
            if (playing) {
    ...
    // ── API ───────────────────────────────────────────────────────────────────
    const toggleNote = useCallback((step, pitch, velocity = 100) => {
    ...
    const setPatternLength = useCallback((steps) => {
        ...
        for (const note of pattern.notes) {
            if (note.step >= steps)
                store.removeMidiNote(activePatternId, note.id);
        }
        // Update steps count via partial pattern mutation — expose via store if needed
    }, []);
    const nudgePitch = useCallback((noteId, delta) => {
    ...
    const nudgeVelocity = useCallback((noteId, delta) => {
        const { activePatternId, updateMidiNote } = useDAWStore.getState();
        if (!activePatternId)
            return;
        const pattern = useDAWStore.getState().midiPatterns.find(p => p.id === activePatternId);
