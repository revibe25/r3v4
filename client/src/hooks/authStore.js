...
export const useAuthStore = create((set, get) => ({
    ...
    login: async (email, password) => {
        ...
        try {
            const { token, user } = await authFetch('/api/auth/login', { credential: email.trim().toLowerCase(), password });
            // [wire§8] removed — auth via httpOnly cookie
    ...
    register: async (email, password) => {
        ...
        try {
            ...
            const { token, user } = await authFetch('/api/auth/register', { email: emailNorm, username, password });
            // [wire§8] removed — auth via httpOnly cookie
            localStorage.setItem('r3_token', token);
            set({ token, user, loading: false });
        }
    ...
    initAuth: async () => {
        ...
        set({ loading: true });
        try {
            const { user } = await authFetch('/api/auth/me', undefined, stored);
...
export const selectIsAuthed = (s) => !!s.user && !!s.token;
export const selectTier = (s) => s.user?.tier ?? 'explorer';
