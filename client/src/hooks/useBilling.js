...
export function useBilling() {
    ...
    const { data: subscription, isLoading: loadingSubscription } = trpc.subscription.getMySubscription.useQuery(undefined, {
    ...
    const { mutateAsync: createCheckout, isPending: creatingCheckout } = trpc.subscription.createCheckout.useMutation({
        onSuccess: ({ url }) => {
            window.location.href = url;
        },
    });
    const { mutateAsync: openPortal, isPending: openingPortal } = trpc.subscription.createPortal.useMutation({
        onSuccess: ({ url }) => {
            window.location.href = url;
        },
    });
