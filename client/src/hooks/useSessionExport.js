 ...
 */
export function useSessionExport(state, exportSession) {
    const exportToJson = useCallback(async (basename) => {
    ...
    const exportToWav = useCallback(async (basename) => {
        try {
    ...
    const exportToMidi = useCallback(async (basename) => {
        try {
            // Simple MIDI export
            // Requires @tonejs/midi or equivalent
            const midi = createMidiFromEvents(state.recordedEvents || [], state.bpm || 120);
 ...
 */
function createMidiFromEvents(events, tempo) {
    ...
    midiEvents.push((microsecondsPerBeat >> 16) & 0xff, (microsecondsPerBeat >> 8) & 0xff, microsecondsPerBeat & 0xff);
    // Add note events
...
let view; // Global for track length update
