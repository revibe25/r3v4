...
import { getLoopEngine, TRACK_COUNT, FFT_SIZE, ANALYSER_SIZE, } from "../features/loopstation/engine/loopEngine";
const BAND_RANGES = {
    sub: [20, 80],
    low: [80, 250],
    mid: [250, 2000],
...
function fftBandEnergy(fft, loHz, hiHz, sr = 44100) {
...
export function useLoopEngineFFT(fps = 60) {
...
export function useLoopEngineFFTRef() {
    const ref = useRef(emptyData());
    const rafRef = useRef(0);
    useEffect(() => {
        const tick = () => {
            rafRef.current = requestAnimationFrame(tick);
