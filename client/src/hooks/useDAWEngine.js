...
export function useDAWEngine() {
    ...
    useEffect(() => {
        ...
        const unsub = useDAWStore.subscribe(s => s.bpm, bpm => { Tone.Transport().bpm.value = bpm; }, { fireImmediately: true });
        return () => unsub();
    }, []);
    useEffect(() => {
        const unsub = useDAWStore.subscribe(s => s.masterGain, gain => {
    ...
    useEffect(() => {
        const unsub = useDAWStore.subscribe(s => s.tracks, tracks => {
