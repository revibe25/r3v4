 ...
 * });
 */
export function usePerformanceMonitor({ enabled, onMetrics, interval = 1000, warningThreshold = 30, }) {
    ...
    const measureMetrics = useCallback(() => {
        ...
        if (fps < warningThreshold && elapsed > 100) {
            console.warn(`[PERF WARNING] Low FPS detected: ${fps} (threshold: ${warningThreshold})`);
        }
        onMetrics?.(metrics);
