 ...
 */
export function useAudioEngine(tracks) {
    // State management
     ...
     */
    const setChannelVolume = useCallback((trackId, volume) => {
        const channel = channelsRef.current.get(trackId);
        channel?.setVolume?.(volume);
    }, []);
    const setChannelPan = useCallback((trackId, pan) => {
        const channel = channelsRef.current.get(trackId);
        channel?.setPan?.(pan);
    }, []);
    const setChannelMute = useCallback((trackId, muted) => {
        const channel = channelsRef.current.get(trackId);
        channel?.setMute?.(muted);
    }, []);
    const setChannelSolo = useCallback((trackId, solo) => {
        const channel = channelsRef.current.get(trackId);
        channel?.setSolo?.(solo);
    }, []);
    const getChannel = useCallback((trackId) => {
    ...
    return {
        ...
        // Global controls
        setBpm: analysisEngine?.setBpm?.bind(analysisEngine) || ((bpm) => {
