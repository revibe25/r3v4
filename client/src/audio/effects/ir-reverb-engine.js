...
export class IRReverbEngine {
    ...
    async load(url) {
        if (!this._ctx || !this._conv)
            throw new Error("[IRReverbEngine] call init() first");
        if (url === this._curUrl && this._loaded)
            return;
        this._loading = true;
        try {
            const res = await fetch(url);
            if (!res.ok)
                throw new Error(`[IRReverbEngine] fetch failed: ${url} (${res.status})`);
    ...
    connectSource(src) { this._input?.connect(src); src.connect(this._input); }
    connectDestination(dest) { this._output?.connect(dest); }
    /**
    ...
    patchIntoLoopEngine(loopEngine) {
        if (!this._input || !this._output)
            throw new Error("[IRReverbEngine] not initialized");
        // Tap master analyser → our input
        loopEngine.masterAnalyser.connect(this._input);
        // Return → master limiter input
        const limIn = loopEngine.masterLimiter.input;
        if (limIn)
            this._output.connect(limIn);
        else
            loopEngine.masterLimiter.connect(this._output);
    }
    setWet(wet, rampMs = 50) {
    ...
    setPreGain(gain) {
        ...
        if (this._preGain)
            this._preGain.gain.setTargetAtTime(this._preG, this._ctx?.currentTime ?? 0, 0.015);
    }
