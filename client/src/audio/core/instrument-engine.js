...
class AudioEngine {
    ...
    async init() {
        ...
        try {
            const workletUrl = new URL('../../worklets/instrument-processor.worklet.ts', import.meta.url);
    ...
    playBuffer(buffer, vol = 1, octaveShift = 0) {
        if (!this.ctx || !buffer)
            return;
        let bufToUse = buffer;
    ...
    triggerKey(index, octaveShift = 0, velocity = 1) {
    ...
    assignPadSample(padIndex, buffer, name) {
    ...
    assignKeySample(keyIndex, buffer, name) {
    ...
    setMSParams(opts) {
        if (opts.width !== undefined)
            this.setMSWidth(opts.width);
        if (opts.midGain !== undefined)
            this.setMidGain(opts.midGain);
        if (opts.sideGain !== undefined)
            this.setSideGain(opts.sideGain);
        if (opts.midThreshold !== undefined)
            this.setMidThreshold(opts.midThreshold);
        if (opts.sideThreshold !== undefined)
            this.setSideThreshold(opts.sideThreshold);
    }
    exportSession() {
    ...
    importSession(json) {
        try {
            ...
            if (data.fx)
                this.state.fx = { ...this.state.fx, ...data.fx };
            if (data.filterVal !== undefined)
                this.state.filterVal = data.filterVal;
            if (data.pitchSemitones !== undefined)
                this.state.pitchSemitones = data.pitchSemitones;
            if (data.recordedEvents)
                this.state.recordedEvents = data.recordedEvents;
            this.notify();
        }
        catch (e) {
