...
export class Crossfader {
     ...
     */
    connectChannelA(source) {
        source.connect(this.channelAGain.input ?? this.channelAGain);
        return this;
    }
    /**
     * Connect channel B input
     */
    connectChannelB(source) {
        source.connect(this.channelBGain.input ?? this.channelBGain);
        return this;
    }
    /**
     * Soft-crossfade to one channel (animate over duration)
     */
    soften(toChannel, duration = 0.5) {
        const targetPosition = toChannel === 'A' ? -1 : 1;
        const now = Tone.now();
        // Use exponential ramp for smooth fade
        this.channelAGain.gain.exponentialRampToValueAtTime(toChannel === 'A' ? 1 : 0.01, now + duration);
        this.channelBGain.gain.exponentialRampToValueAtTime(toChannel === 'B' ? 1 : 0.01, now + duration);
