...
export class VSTFXNode extends FXNodeBase {
    ...
    async initialize() {
        try {
            ...
            this.vstNode.port.onmessage = (event) => {
    ...
    setupMessageHandlers() {
        ...
        this.messageHandlers.set('state', (data) => {
            Object.entries(data.parameters).forEach(([id, value]) => {
                this.parameterValues.set(Number(id), value);
            });
        });
    ...
    async getParameterFromWorklet(paramId) {
        if (!this.vstNode)
            return 0;
    ...
    getAutomation(paramId) {
        return this.automationLanes.get(paramId) ?? null;
    }
    enableAutomation(paramId, enabled) {
    ...
    /* ===========================
       Preset Management
    ============================ */
    async savePreset(name, metadata) {
        const parameters = {};
    ...
    /* ===========================
       Hot Swap
    ============================ */
    async hotSwap(newVstUrl, preserveState = true) {
        if (!this.vstNode) {
    ...
    static async deserialize(data, audioContext) {
        const node = new VSTFXNode(audioContext, data.config);
        await node.initialize();
        // Restore parameters
        Object.entries(data.parameters).forEach(([id, value]) => {
            node.setParameter(Number(id), value);
        });
        // Restore automation
        data.automation?.forEach((lane) => {
