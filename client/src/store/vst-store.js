...
export const useVSTStore = create()(devtools((set, get) => ({
    ...initialState,
    // ============================================
    // INITIALIZATION
    // ============================================
    initialize: async (audioContext) => {
        if (get().initialized) {
        ...
        try {
            ...
            const [{ VSTPerformanceMonitor }, { SidechainRouter }, { VSTAutomationEngine },] = await Promise.all([
...
// SELECTORS
// ============================================
export const selectCPUUsage = (state) => state.currentSnapshot?.cpu.total ?? 0;
export const selectMemoryUsage = (state) => state.currentSnapshot?.memory.percentage ?? 0;
export const selectLatency = (state) => state.currentSnapshot?.latency.total ?? 0;
export const selectHasAlerts = (state) => state.cpuOverload || state.memoryWarning || state.dropoutCount > 0;
export const selectChannelFX = (channelId) => (state) => state.channelFXChains[channelId] ?? [];
export const selectActivePlugin = (state) => state.activePlugin;
export const selectPerformanceHistory = (state, duration = 10) => {
    const now = Date.now();
