...
export async function getUploadConfig(): Promise<UploadServerConfig> {
  if (_cfg) return _cfg;
  const _res = await fetch('/api/uploads/config');
...
export async function validateFile(file: File): Promise<string | null> {
  const _cfg = await getUploadConfig();
...
async function uploadSingle(file: File, options: UploadOptions): Promise<UploadResult> {
  const _res = await fetch('/api/uploads/sign', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ filename: file.name, contentType: file.type, size: file.size }) });
...
async function uploadMultipart(file: File, options: UploadOptions): Promise<UploadResult> {
  const _initRes = await fetch('/api/uploads/multipart/init', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ filename: file.name, contentType: file.type, size: file.size }) });
  ...
  let _uploaded = 0;
  try {
    for (let _i = 1; i <= totalParts; i++) {
      const _chunk = file.slice((i-1)*partSize, Math.min(i*partSize, file.size));
      const _pRes = await fetch('/api/uploads/multipart/part', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fileKey, uploadId, partNumber: i }) });
      const { uploadUrl } = await pRes.json();
      const _etag = await xhrUpload(uploadUrl, chunk, file.type, (_, l) => { options.onProgress?.(Math.round((uploaded+l)/file.size*100), uploaded+l, file.size); });
      uploaded += chunk.size;
      parts.push({ PartNumber: i, ETag: etag });
    }
    const _cRes = await fetch('/api/uploads/multipart/complete', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fileKey, uploadId, parts }) });
...
export async function uploadAudioFile(file: File, options: UploadOptions = {}): Promise<UploadResult> {
  const _validErr = await validateFile(file);
