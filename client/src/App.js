...
function MultitrackViewWrapper() {
    ...
    return (_jsx(MultitrackView, { tracks: tracks.map(adaptTrack), transport: { isPlaying: playing, isRecording: recording, position }, hideTransport: true, onTogglePlay: () => setPlaying(!playing), onToggleRecord: () => setRecording(!recording), onArmTrack: (id) => updateTrack(id, { armed: !tracks.find(t => t.id === id)?.armed }), onToggleMute: (id) => updateTrack(id, { mute: !tracks.find(t => t.id === id)?.mute }), onToggleSolo: (id) => updateTrack(id, { solo: !tracks.find(t => t.id === id)?.solo }), onUpdateTrack: (id, data) => {
            const patch = {};
...
export default function App() {
    ...
    return (_jsx(TRPCProvider, { children: _jsx(SubscriptionProvider, { children: _jsx(TrialGate, { children: _jsxs(ThemeProvider, { children: [_jsx("style", { children: `:root { --nav-h: ${NAV_HEIGHT_PX}px; }` }), _jsxs("div", { style: {
                                display: 'flex',
