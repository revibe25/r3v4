...
const STRENGTH_LABEL = ['', 'WEAK', 'FAIR', 'STRONG', 'MAX'];
const STRENGTH_COLOR = ['', '#ff3b3b', 'var(--status-warn)', '#a3e635', 'var(--accent-cyan)'];
// ─── Keyframe injection ───────────────────────────────────────────────────────
...
function PulseWaveCanvas() {
    ...
    useEffect(() => {
        ...
        let resizeTimer; // window.setTimeout returns number; @types/node was hijacking ReturnType<>
        const onResize = () => {
...
function MiniOscilloscope({ active }) {
    const canvasRef = useRef(null);
    const bufRef = useRef(new Float32Array(200));
    const tRef = useRef(0);
    useEffect(() => {
        if (!active)
            return;
        ...
        const draw = () => {
            ...
            const newSample = Math.sin(t * 0.12) * 0.4
    ...
    return (_jsx("canvas", { ref: canvasRef, width: 220, height: 36, style: { width: '100%', height: 36, display: 'block' } }));
}
// ─── LLPTE status badge row ───────────────────────────────────────────────────
function LLPTEBar({ state }) {
    const isActive = state === 'loading' || state === 'success';
    const latency = isActive ? '10ms' : '—';
    const edges = isActive ? '847' : '—';
    const badge = (label, val, color) => (_jsxs("div", { style: {
            display: 'flex',
            alignItems: 'center',
            gap: 5,
            padding: '3px 8px',
            ...
            background: 'var(--t-b0x)',
                ...
                } }), _jsx("span", { style: { fontSize: 8, color: T.muted, letterSpacing: '.2em' }, children: label }), _jsx("span", { style: { fontSize: 8, color: isActive ? color : T.dim, letterSpacing: '.1em', fontWeight: 700 }, children: val })] }));
    return (_jsxs("div", { style: {
            ...
            overflowX: 'auto',
        }, children: [_jsx(Cpu, { size: 9, color: T.muted, strokeWidth: 1.5, style: { flexShrink: 0 } }), _jsx("span", { style: { fontSize: 8, letterSpacing: '.25em', color: T.dim, marginRight: 4 }, children: "LLPTE" }), badge('INFER', latency, T.cyan), badge('EDGES', edges, T.violet), badge('TICK', isActive ? '0.8ms' : '—', T.accent), _jsx("div", { style: { flex: 1 } }), _jsx(Activity, { size: 9, color: isActive ? T.cyan : T.dim, strokeWidth: 1.5, style: { flexShrink: 0 } })] }));
}
// ─── Mini oscilloscope panel ──────────────────────────────────────────────────
function OscilloscopePanel({ active }) {
    return (_jsxs("div", { style: {
            ...
            background: 'var(--t-b0xx)',
                    ...
                    alignItems: 'center',
                    marginBottom: 6,
                }, children: [_jsx("span", { style: { fontSize: 8, color: T.dim, letterSpacing: '.2em' }, children: "SIGNAL" }), _jsxs("div", { style: { display: 'flex', gap: 10 }, children: [_jsx("span", { style: { fontSize: 7, color: T.violet, letterSpacing: '.15em' }, children: "\u25A0 PRE-AI" }), _jsx("span", { style: { fontSize: 7, color: T.cyan, letterSpacing: '.15em' }, children: "\u25A0 POST-AI" })] })] }), _jsx("div", { style: {
                    background: T.bg,
                    border: `1px solid ${T.border}`,
                    overflow: 'hidden',
                }, children: _jsx(MiniOscilloscope, { active: active }) })] }));
}
// ─── Submit button ────────────────────────────────────────────────────────────
function SubmitButton({ state }) {
    ...
    const bg = isSuccess
        ...
        : hovered && !isLoading
            ? T.accentHv
            : T.accent;
    return (_jsxs("button", { type: "submit", disabled: isLoading || isSuccess, onMouseEnter: () => setHovered(true), onMouseLeave: () => setHovered(false), style: {
            display: 'flex',
            alignItems: 'center',
            ...
            position: 'relative',
            overflow: 'hidden',
        }, children: [hovered && !isLoading && !isSuccess && (_jsx("div", { style: {
                    position: 'absolute',
                    inset: 0,
                    background: 'linear-gradient(90deg, transparent 0%, rgba(255,255,255,0.12) 50%, transparent 100%)',
                    animation: 'r3-scan 0.6s ease-out forwards',
                    pointerEvents: 'none',
                } })), isSuccess ? (_jsxs(_Fragment, { children: [_jsx("svg", { width: "13", height: "13", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2.5", children: _jsx("polyline", { points: "20 6 9 17 4 12" }) }), "AUTHORIZED"] })) : isLoading ? (_jsxs(_Fragment, { children: [_jsx("svg", { className: "r3-spin", width: "13", height: "13", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2.5", children: _jsx("circle", { cx: "12", cy: "12", r: "10", strokeDasharray: "40", strokeDashoffset: "30", strokeLinecap: "round" }) }), "AUTHENTICATING\u2026"] })) : (_jsxs(_Fragment, { children: [_jsx(LogIn, { size: 13, strokeWidth: 2 }), "AUTHENTICATE"] }))] }));
}
// ─── Field block ──────────────────────────────────────────────────────────────
function FieldBlock({ label, children, style, }) {
    return (_jsxs("div", { style: { marginBottom: 20, ...style }, children: [_jsx("span", { style: {
                    display: 'block',
                    fontSize: 9,
                    letterSpacing: '.28em',
                    textTransform: 'uppercase',
...
export default function LoginPage() {
    ...
    return (_jsxs("div", { style: {
            ...
            position: 'relative',
            overflow: 'hidden',
        }, children: [_jsx(PulseWaveCanvas, {}), _jsx("div", { style: {
                    position: 'absolute',
                    inset: 0,
                    background: 'radial-gradient(ellipse at center, transparent 40%, rgba(0,0,0,0.6) 100%)',
                    pointerEvents: 'none',
                } }), _jsxs("div", { className: `r3-glow-card${shakeKey > 0 ? ' r3-shake' : ''}`, style: {
                    position: 'relative',
                    zIndex: 10,
                    ...
                    transition: 'opacity 0.45s ease, transform 0.45s cubic-bezier(.22,1,.36,1), border-top-color 0.25s',
                            ...
                            gap: 14,
                                    ...
                                    animation: 'r3-pulse-dot 2s ease-in-out infinite',
                                } }), _jsx(LogIn, { size: 15, color: isSuccess ? T.cyan : T.accent, strokeWidth: 1.5, style: { transition: 'color 0.2s' } }), _jsxs("div", { style: { flex: 1 }, children: [_jsxs("div", { style: {
                                            fontSize: 15,
                                            fontWeight: 700,
                                            color: T.text,
                                            letterSpacing: '-.01em',
                                            lineHeight: 1,
                                            marginBottom: 6,
                                        }, children: ["R3", _jsx("span", { style: { color: T.accent, margin: '0 3px' }, children: "/" }), "NATIVE", _jsx("span", { style: {
                                                    marginLeft: 10,
                                                    fontSize: 9,
                                                    letterSpacing: '.2em',
                                                    color: T.dim,
                                                    fontWeight: 400,
                                                }, children: "v4.2.1" })] }), _jsx("div", { style: {
                                            fontSize: 9,
                                            letterSpacing: '.3em',
                                            textTransform: 'uppercase',
                                            color: T.muted,
                                        }, children: "Authenticate to continue" })] }), _jsxs("div", { style: {
                                    display: 'flex',
                                    alignItems: 'center',
                                    gap: 6,
                                    fontSize: 8,
                                    ...
                                    transition: 'color 0.2s',
                                        ...
                                        } })), isSuccess ? 'AUTHORIZED' : isError ? 'DENIED' : isLoading ? 'CHECKING' : 'SECURE'] })] }), _jsx(LLPTEBar, { state: loginState }), _jsx(OscilloscopePanel, { active: isLoading || isSuccess }), _jsxs("form", { onSubmit: handleSubmit, style: { padding: '28px 36px 32px' }, noValidate: true, children: [isError && errorMsg && (_jsxs("div", { className: "r3-fadein", role: "alert", style: {
                                    display: 'flex',
                                    gap: 10,
                                    ...
                                    padding: '12px 16px',
                                    marginBottom: 24,
                                }, children: [_jsx(AlertCircle, { size: 12, color: T.error, style: { marginTop: 1, flexShrink: 0 } }), _jsx("span", { style: { fontSize: 11, color: 'rgba(255,59,59,.8)', letterSpacing: '.05em', lineHeight: 1.5 }, children: errorMsg })] })), _jsx(FieldBlock, { label: "Email or Username", children: _jsx("input", { ref: credRef, type: "text", value: credential, onChange: e => setCredential(e.target.value), placeholder: "you@example.com", autoComplete: "username", spellCheck: false, disabled: isLoading || isSuccess, className: "r3-input" }) }), _jsx(FieldBlock, { label: "Password", style: { marginBottom: 8 }, children: _jsxs("div", { style: { position: 'relative' }, children: [_jsx("input", { type: showPw ? 'text' : 'password', value: password, onChange: e => setPassword(e.target.value), placeholder: "\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022", autoComplete: "current-password", disabled: isLoading || isSuccess, className: "r3-input", style: { paddingRight: 44 } }), _jsx("button", { type: "button", onClick: () => setShowPw(v => !v), "aria-label": showPw ? 'Hide password' : 'Show password', style: {
                                                position: 'absolute',
                                                right: 12,
                                                top: '50%',
                                                ...
                                                display: 'flex',
                                                alignItems: 'center',
                                            }, children: showPw ? _jsx(EyeOff, { size: 13, strokeWidth: 1.5 }) : _jsx(Eye, { size: 13, strokeWidth: 1.5 }) })] }) }), password.length > 0 && (_jsxs("div", { style: { display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20 }, children: [_jsx("div", { style: { display: 'flex', gap: 3, flex: 1 }, children: [1, 2, 3, 4].map(n => (_jsx("div", { style: {
                                                flex: 1,
                                                height: 2,
                                        ...
                                        }, children: STRENGTH_LABEL[strength] })] })), _jsxs("div", { style: {
                                    display: 'flex',
                                    alignItems: 'center',
                                    justifyContent: 'space-between',
                                    marginBottom: 28,
                                            ...
                                            cursor: 'pointer',
                                            userSelect: 'none',
                                        }, children: [_jsx("input", { type: "checkbox", checked: remember, onChange: e => setRemember(e.target.checked), disabled: isLoading || isSuccess, className: "r3-checkbox" }), _jsx("span", { style: { fontSize: 9, letterSpacing: '.2em', textTransform: 'uppercase', color: T.muted }, children: "Remember me" })] }), _jsx(Link, { href: "/forgot-password", style: {
                                            fontSize: 9,
                                            letterSpacing: '.15em',
                                            textTransform: 'uppercase',
                                            color: T.dim,
                                        ...
                                        }, children: "Forgot password?" })] }), _jsx(SubmitButton, { state: loginState }), _jsxs("div", { style: { display: 'flex', alignItems: 'center', gap: 14, margin: '24px 0' }, children: [_jsx("div", { style: { flex: 1, height: 1, background: T.border } }), _jsx("span", { style: { fontSize: 8, letterSpacing: '.25em', color: T.dim, textTransform: 'uppercase' }, children: "or" }), _jsx("div", { style: { flex: 1, height: 1, background: T.border } })] }), _jsxs("p", { style: { fontSize: 10, letterSpacing: '.1em', color: T.muted, textAlign: 'center', margin: 0 }, children: ["No account?", ' ', _jsx(Link, { href: "/", style: { color: T.accent, textDecoration: 'none' }, children: "View Plans \u2192" }), ' ', "\u00B7", ' ', _jsx(Link, { href: "/auth", style: { color: T.violet, textDecoration: 'none' }, children: "Full Auth \u2192" })] })] }), _jsxs("div", { style: {
                            display: 'flex',
                            justifyContent: 'space-between',
                            alignItems: 'center',
                            padding: '10px 36px',
                            ...
                            background: 'var(--t-b0xx)',
                        }, children: [_jsx("span", { style: { fontSize: 7, letterSpacing: '.2em', color: T.dim }, children: "SECURE \u00B7 ENCRYPTED" }), _jsx("div", { style: { display: 'flex', gap: 6, alignItems: 'center' }, children: [T.cyan, T.violet, T.violet, T.cyan, 'var(--status-ok)'].map((c, i) => (_jsx("div", { style: {
                                        width: 5,
                                        height: 5,
                                        borderRadius: '50%',
                                        background: c,
                                        opacity: 0.5,
                                    } }, i))) }), _jsx("span", { style: { fontSize: 7, letterSpacing: '.2em', color: T.dim }, children: "R3 v4" })] }), _jsx("div", { style: {
                            height: 2,
                            background: `repeating-linear-gradient(90deg, ${T.border} 0px, ${T.border} 1px, transparent 1px, transparent 5px)`,
                            opacity: 0.4,
                        } })] }, shakeKey === 0 ? 'card-init' : `card-${shakeKey}`)] }));
}
