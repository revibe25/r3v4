...
import { Mic, AlertCircle, Activity, SlidersHorizontal, Headphones, Music, Music2, Repeat2 } from 'lucide-react';
...
const WaveformEditor = lazy(() => import('@/components/waveform-editor').then(m => ({ default: m.WaveformEditor })));
...
// COMPONENT
// ─────────────────────────────────────────────────────────────────────────────
export default function InstrumentPage({ autoInitialize = false, defaultBpm = 120, } = {}) {
    const { toast } = useToast();
    const [initError, setInitError] = useState(null);
    const [isLoading, setIsLoading] = useState(false);
    const tapTimesRef = useRef([]);
    const [tapFlash, setTapFlash] = useState(false);
    const { state, isInitialized, init, triggerPad, triggerKey, toggleFX, setFilter, setPitch, setCrossfade, setBpm, toggleMetronome, arm, record, stop, play, undo, redo, getAnalyserData, getWaveformData, loadSample, assignPadSample, assignKeySample, exportSession, importSession, } = useAudioEngine();
    ...
    useEffect(() => {
        ...
        const onClick = () => { if (!isInitialized && !isLoading)
            go(); };
        const onKeyDown = () => { if (!isInitialized && !isLoading)
            go(); };
    ...
    useEffect(() => {
        ...
        const h = (e) => {
            ...
            if (pi !== -1 && state.pads[pi]) {
                e.preventDefault();
                triggerPad(pi);
                return;
            }
            const ki = KEYBOARD_SHORTCUTS.keys.indexOf(k);
            if (ki !== -1 && state.keys[ki]) {
                e.preventDefault();
                triggerKey(ki);
                return;
            }
    ...
    // ────────────────────────────────────────────────────────────
    const handleMidiKey = useCallback((index, octaveShift, velocity) => {
    ...
    const handleLoadJson = useCallback(async (json) => {
        try {
            setIsLoading(true);
            try {
                JSON.parse(json);
    ...
    useEffect(() => {
        ...
        autoSaveTimer.current = setTimeout(() => {
            ...
            saveSession(exportSession()).catch(() => { });
        }, 2000);
        return () => {
            if (autoSaveTimer.current)
                clearTimeout(autoSaveTimer.current);
    ...
    // ── Render ────────────────────────────────────────────────────────────────
    return (_jsxs(_Fragment, { children: [_jsx("style", { children: STYLES }), _jsxs("div", { className: "ag-shell", children: [_jsxs("div", { className: "ag-frame", children: [initError && (_jsxs("div", { className: "ag-error", children: [_jsx(AlertCircle, { size: 14, color: "var(--ag-err)", style: { flexShrink: 0, marginTop: 1 } }), _jsxs("div", { children: [_jsx("div", { className: "ag-error-title", children: "ERR / INIT FAILED" }), _jsx("div", { className: "ag-error-desc", children: initError }), _jsx("button", { className: "ag-error-btn", onClick: () => { setInitError(null); init(); }, children: "RETRY" })] })] })), _jsxs("header", { className: "ag-header", children: [_jsxs("div", { className: "ag-header-top", children: [_jsxs("div", { className: "ag-wordmark-block", children: [_jsxs("div", { className: "ag-wordmark", "data-testid": "text-title", children: ["R3", _jsx("span", { className: "ag-wordmark-slash", children: "/" }), "NATIVE"] }), _jsx("div", { className: "ag-wordmark-sub", children: "Instrument \u00B7 Virtual VSTs" })] }), _jsxs("div", { className: "ag-status-block", children: [_jsxs("div", { className: `ag-status-line ${isInitialized ? 'ag-status-live-text' : 'ag-status-dead-text'}`, children: [_jsx("span", { className: isInitialized ? 'ag-cursor-live' : 'ag-cursor-standby' }), isInitialized ? 'LIVE' : 'STANDBY'] }), _jsx("div", { className: "ag-status-line", style: { color: 'var(--ag-white)' }, children: "ERNESTO \u00B7 R3VIBE" }), _jsxs("div", { className: "ag-status-line", style: {
                                                            color: midiStatus === 'active' ? 'var(--ag-acid)'
                                                                : midiStatus === 'denied' ? 'var(--ag-err)'
                                                                    : 'var(--ag-mid)',
                                                        }, children: ["MIDI\u00A0", (midiStatus ?? 'idle').toUpperCase(), midiInputCount > 0 ? ` (${midiInputCount})` : ''] })] }), _jsxs("div", { className: "ag-bpm-block", children: [_jsx("span", { className: "ag-bpm-label", children: "BPM" }), _jsx("span", { className: "ag-bpm-number", children: isInitialized ? state.bpm : '120' }), _jsx("button", { onClick: handleTapTempo, style: {
                                                            ...
                                                            flexShrink: 0,
                                                        }, title: "Tap Tempo (tap 2\u20138\u00D7 to set BPM)", children: "TAP" })] }), _jsx("span", { className: "ag-ghost-bpm", "aria-hidden": "true", children: isInitialized ? state.bpm : '120' }), _jsx("div", { className: "ag-controls-block", children: _jsx(Link, { href: "/daw", className: "ag-nav-btn", children: "\uD83C\uDF9A DAW" }) })] }), _jsx(HeaderControls, { bpm: state.bpm, onBpmChange: setBpm, metronomeOn: state.metronomeOn, onMetronomeToggle: toggleMetronome, onSave: handleSave, onLoad: handleLoadJson, getSessionData: getSessionData }), _jsx("div", { className: "ag-ticker-row", children: _jsx("div", { className: "ag-ticker-inner", children: [...TICKER, ...TICKER].map((item, i) => (_jsxs("span", { className: "ag-ticker-item", children: [item, _jsx("span", { className: "ag-ticker-sep", children: "/" })] }, i))) }) })] }), _jsxs("div", { className: "ag-content", children: [_jsxs("div", { className: "ag-left", children: [_jsxs("div", { className: "ag-section-strip", children: [_jsx("span", { className: "ag-section-tag", children: "01 \u2014 Instruments" }), _jsx("span", { className: "ag-section-line" })] }), [
                                                { num: '01', title: 'Drum Pads', maxHeight: 400, icon: _jsx(Music, { className: "h-4 w-4" }), open: true,
                                                    body: _jsx(DrumPads, { pads: state.pads, onTrigger: triggerPad, onAssignSample: assignPadSample, loadSample: handleLoadSample, disabled: !isInitialized }) },
                                                { num: '02', title: 'Piano Keys', maxHeight: 280, icon: _jsx(Music2, { className: "h-4 w-4" }), open: true,
                                                    body: _jsx(PianoKeys, { keys: state.keys, onTrigger: triggerKey, onAssignSample: assignKeySample, loadSample: handleLoadSample, disabled: !isInitialized }) },
                                                { num: '03', title: 'Waveform Editor', maxHeight: 360, icon: _jsx(Activity, { className: "h-4 w-4" }), open: false,
                                                    body: _jsx(Suspense, { fallback: _jsx("div", { style: { padding: '20px', color: 'var(--ag-mid)', fontSize: 10, letterSpacing: '.15em', fontFamily: "'IBM Plex Mono',monospace" }, children: "LOADING..." }), children: _jsx(WaveformEditor, { getWaveformData: getWaveformData, isInitialized: isInitialized }) }) },
                                                { num: '03B', title: 'VST Browser', maxHeight: 400, icon: _jsx(Music2, { className: "h-4 w-4" }), open: false,
                                                    body: (_jsx(VSTBrowser, { onPluginSelect: (plugin) => {
                                                            toast({ title: 'Plugin Selected', description: `${plugin.name} by ${plugin.vendor}` });
                                                        } })) },
                                                { num: '04', title: 'Loop Station', maxHeight: 440, icon: _jsx(Repeat2, { className: "h-4 w-4" }), open: false,
                                                    body: _jsx(Suspense, { fallback: _jsx("div", { style: { padding: '20px', color: 'var(--ag-mid)', fontSize: 10, letterSpacing: '.15em', fontFamily: "'IBM Plex Mono',monospace" }, children: "LOADING..." }), children: _jsx(LoopStation505, {}) }) },
                                            ].map(({ num, title, icon, open, body, maxHeight }) => (_jsxs("div", { className: "ag-panel", children: [_jsx("span", { className: "ag-panel-ghost", "aria-hidden": "true", children: num }), _jsx(CollapsibleFXPanel, { title: title, icon: icon, defaultOpen: open, variant: "default", scrollable: true, maxHeight: maxHeight ?? 360, children: body })] }, num)))] }), _jsxs("div", { className: "ag-right", children: [_jsxs("div", { className: "ag-section-strip", children: [_jsx("span", { className: "ag-section-tag", children: "02 \u2014 Controls" }), _jsx("span", { className: "ag-section-line" })] }), [
                                                { num: '05', title: 'Visualizer & Transport', maxHeight: 340, icon: _jsx(Activity, { className: "h-4 w-4" }), open: true,
                                                    body: (_jsxs(_Fragment, { children: [_jsx(AudioVisualizer, { getAnalyserData: getAnalyserData, isInitialized: isInitialized, isActive: isInitialized }), _jsx("div", { className: "mt-4", children: _jsx(TransportControls, { isArmed: state.isArmed, isRecording: state.isRecording, isPlaying: state.isPlaying, recordedEventsCount: state.recordedEvents.length, onArm: arm, onRecord: record, onStop: stop, onPlay: play, onUndo: undo, onRedo: redo, onExport: handleExport, disabled: !isInitialized }) })] })) },
                                                { num: '06', title: 'Microphone Input', maxHeight: 260, icon: _jsx(Mic, { className: "h-4 w-4" }), open: false,
                                                    body: _jsx(MicrophoneInput, { onAudioData: handleMicrophoneData }) },
                                                { num: '07', title: 'FX Chain', maxHeight: 320, icon: _jsx(SlidersHorizontal, { className: "h-4 w-4" }), open: false,
                                                    body: _jsx(FXPanel, { fx: state.fx, onToggle: toggleFX }) },
                                                { num: '08', title: 'DJ Controls', maxHeight: 400, icon: _jsx(Headphones, { className: "h-4 w-4" }), open: false,
                                                    body: _jsx(DJControls, { filterVal: state.filterVal, pitchSemitones: state.pitchSemitones, crossfade: state.crossfade, onFilterChange: setFilter, onPitchChange: setPitch, onCrossfadeChange: setCrossfade }) },
                                            ].map(({ num, title, icon, open, body, maxHeight }) => (_jsxs("div", { className: "ag-panel", children: [_jsx("span", { className: "ag-panel-ghost", "aria-hidden": "true", children: num }), _jsx(CollapsibleFXPanel, { title: title, icon: icon, defaultOpen: open, variant: "default", scrollable: true, maxHeight: maxHeight ?? 360, children: body })] }, num))), _jsxs("div", { className: "ag-guide", children: [_jsx("div", { className: "ag-guide-header", children: "Reference" }), [['Play', 'Click pads/keys or keyboard'], ['Upload', 'Custom samples via upload'],
                                                        ['Record', 'Arm → Record → Play → Stop'], ['Effects', 'FX + DJ controls live'],
                                                        ['DAW', 'MultiTrack for advanced mix'], ['Save', 'Export session anytime']
                                                    ].map(([k, v]) => (_jsxs("div", { className: "ag-guide-row", children: [_jsx("span", { className: "ag-guide-key", children: k }), _jsx("span", { className: "ag-guide-val", children: v })] }, k))), _jsx("div", { className: "ag-kbd-section", children: [['Pads', 'Q–]'], ['Keys', 'Z–M'], ['Play', 'SPC'], ['Rec', 'Ctrl+R']].map(([l, t]) => (_jsxs("div", { className: "ag-kbd-row", children: [_jsx("span", { className: "ag-kbd-label", children: l }), _jsx("span", { className: "ag-kbd-tag", children: t })] }, l))) })] })] })] }), _jsxs("footer", { className: "ag-footer", children: [_jsx("div", { className: "ag-footer-left", "data-testid": "text-features", children: ['Polyphony', 'Accessible', 'Mobile-Friendly', 'Offline-First', 'MultiTrack DAW', 'VST System'].map(f => (_jsx("span", { className: "ag-footer-feat", children: f }, f))) }), _jsxs("div", { className: "ag-footer-right", "data-testid": "text-tech", children: ["Web Audio API \u00B7 AudioWorklet \u00B7 Web MIDI API \u00B7 IndexedDB \u00B7 LLPTE", _jsx("span", { className: "ag-ver-tag", children: "v4.1" })] })] })] }), !isInitialized && !autoInitialize && (_jsx("div", { className: "ag-overlay", children: _jsxs("div", { className: "ag-init-box", children: [_jsxs("div", { className: "ag-init-title-row", children: [_jsxs("div", { className: "ag-init-big", children: ["R3", _jsx("span", { children: "/" }), "NATIVE"] }), _jsx("div", { className: "ag-init-label", children: isLoading ? 'LOADING' : 'INSTRUMENT' })] }), _jsxs("div", { className: "ag-init-body", children: [_jsx("div", { className: "ag-init-desc", children: isLoading
                                                ? 'Calibrating audio context.\nInitializing DSP subsystems...'
                                                : 'Click anywhere or press any key\nto initialize the audio engine\nand begin your session.' }), !isLoading && _jsx("div", { className: "ag-init-cta", children: "CLICK OR PRESS ANY KEY" }), isLoading && (_jsxs(_Fragment, { children: [_jsx("div", { className: "ag-init-loader" }), _jsx("div", { className: "ag-init-status", children: "Initializing audio context..." })] }))] })] }) })), isLoading && isInitialized && (_jsx("div", { className: "ag-op-overlay", children: _jsxs("div", { className: "ag-op-box", children: [_jsx("div", { className: "ag-op-bar" }), _jsx("span", { className: "ag-op-text", children: "Processing" })] }) }))] })] }));
}
