...
export default function WaveLabProduction() {
  ...
  const [dragState, setDragState] = useState<Record<string, unknown> | null>(null);
  ...
  useEffect(() => {
    // Use the shared AudioContext singleton — never create or close it directly.
    // Lifecycle is owned by MasterEngine (import.meta.hot.dispose teardown).
    // Components must not close a shared AudioContext — doing so destroys it
    // for all consumers for the lifetime of the tab.
      ...
      const animate = () => {
        ...
        if (loopEnabled) {
          ...
          while (barsElapsed >= loopRegion.end) {
            ...
            if (startTimeRef.current !== null) startTimeRef.current += (loopLength * TIMELINE.beatsPerBar * 60 / project.tempo * 1000);
          }
        }
  ...
  const handleCanvasClick = (e) => {
    ...
    let clickedClip: string | null = null;
    project.tracks.forEach((track, index) => {
    ...
    if (clickedClip) {
      const _clip = clickedClip; // capture narrowed string — closures don't narrow mutable lets
      if (e.metaKey || e.ctrlKey) {
        // Toggle selection
        setSelectedClipIds(prev =>
          prev.includes(_clip)
            ? prev.filter(id => id !== _clip)
            : [...prev, _clip]
        );
      } else if (!selectedClipIds.includes(_clip)) {
        setSelectedClipIds([_clip]);
      }
    } else {
      setSelectedClipIds([]);
    }
...
const IconButton = ({
  ...
  <button
    ...
    onMouseLeave={(e) => {
      if (!disabled) {
        e.currentTarget.style.background = (style?.background as string | undefined) || COLORS.surfaceHover;
