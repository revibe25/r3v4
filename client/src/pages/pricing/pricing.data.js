 ...
 * for display only.
 */
import { TIER_DEFINITIONS, SUBSCRIPTION_TIERS, } from '../../../../shared/subscription.types';
// ─── Helpers (pure functions, no React, unit-testable) ────────────────────────
export const resolvePrice = (p, c) => c === "annual" ? p.annualPrice : p.monthlyPrice;
export const isCustomPricing = (p) => p.monthlyPrice === -1;
export const isFree = (p) => p.monthlyPrice === 0;
export const annualTotal = (p) => p.annualPrice * 12;
...
export const PLANS = SUBSCRIPTION_TIERS.map(tierId => {
    ...
    return {
        ...
        badge: def.badge,
        popular: tierId === 'creator' ? true : undefined,
