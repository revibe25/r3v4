...
export function useLoopStation505() {
    ...
    const [fxState, setFxState] = useState(DEFAULT_FX);
    ...
    // ── Tracks ─────────────────────────────────────────────────────────────────
    const pressTrack = useCallback((trackId) => {
        dispatch({
            type: 'SET_TRACKS',
    ...
    const setTrackVolume = useCallback((trackId, v) => {
        dispatch({ type: 'SET_TRACKS',
            tracks: state.tracks.map(t => t.id === trackId ? { ...t, volume: v } : t) });
    }, [state.tracks]);
    const setTrackPan = useCallback((trackId, v) => {
        dispatch({ type: 'SET_TRACKS',
    ...
    return {
        ...
        fx: fxState, midiSync: false, midiInputEnabled: false, midiInputs: [],
        ...
        setTrackVolume, setTrackPan, setTrackEQ: (_s, _v) => { },
        ...
        setReverb: noop2nn, setCompressor: noop2nn, setXY: (_p) => { },
        // MIDI
