...
export const ANALYSER_SIZE = 1024;
export const FFT_SIZE = 2048;
export const TAP_HISTORY = 6;
export const UNDO_DEPTH = 8;
export const LFO_COUNT = 4;
export const MACRO_COUNT = 4;
export const GRAIN_COUNT_MAX = 8;
...
const SC_UPDATE_INTERVAL = '16n';
// Granular defaults
const GRAIN_SIZE_DEFAULT = 0.12; // seconds
const GRAIN_COUNT_DEFAULT = 4;
const GRAIN_INTERVAL = '16n';
const HARMONY_SEMITONES = {
...
const SYNCED_LFO_RATES = ['1m', '2n', '4n', '8n', '16n', '8t', '4t', '1/32'];
let Tone = null;
// ── Internal Helpers ──────────────────────────────────────────────────────────
function clamp(v, lo, hi) {
    return v < lo ? lo : v > hi ? hi : v;
}
function lerpParam(param, to, ramp = 0.05) {
    try {
        param.rampTo(to, ramp);
    }
    catch {
        param.value = to;
    }
...
class LoopEngine {
    constructor() {
        ...
        // Pre-init cache
        this._pendingBpm = 120;
        this._tapTimes = [];
        this._beatCount = 0;
        this._soloActive = false;
        this._metronomeOn = false;
        this._metronomeVol = 0.4;
        this._beatScheduleId = -1;
        this._quantScheduleId = -1;
        this._currentBar = 0;
        this._currentBeat = 0;
        this._swingAmount = 0;
        this._quantMode = '1m';
        ...
        this._midiInputPort = undefined;
        this._midiInputEnabled = false;
        ...
        this._exciterHPF = null;
        this._exciterSat = null;
        this._exciterWetGain = null;
        this._exciterDryGain = null;
        this._exciterSumBus = null;
        this._exciterState = { amount: 0, tone: 6000, drive: 0.4 };
        this._exciterEnabled = false;
        this._sidechainScheduleId = -1;
        this._sidechainSourceIdx = -1;
        this._sidechainAmount = 0;
        this._sidechainAttack = 0.003;
        this._sidechainRelease = 0.15;
        this._sidechainEnvLevel = 1.0; // running gain (attack/release smoothed)
        ...
        this._mbEnabled = false;
        this._mbLowLP = null; // LP at lowFreq
        this._mbMidHP = null; // HP at lowFreq (mid input)
        this._mbMidLP = null; // LP at highFreq (mid output)
        this._mbHighHP = null; // HP at highFreq
        this._mbLowComp = null;
        this._mbMidComp = null;
        this._mbHighComp = null;
        this._mbLowGain = null;
        this._mbMidGain = null;
        this._mbHighGain = null;
        this._mbSumBus = null;
        this._mbState = {
            enabled: false,
        ...
        this._granularFrozen = false;
        this._granularSourceTrack = 0;
        ...
        this._preMonoWidth = 0.5;
        this._limiterEnabled = true;
        this._preBypassLimiterThreshold = -1;
        this._listeners = {};
    ...
    static getInstance() {
        if (!LoopEngine._instance)
            LoopEngine._instance = new LoopEngine();
    ...
    emit(event, ...args) {
        this._listeners[event]
            ...
            console.error('[LoopEngine] event handler error', e);
        } });
    }
    // ── init() ────────────────────────────────────────────────────────────────
    async init() {
        ...
        try {
            ...
            //   → limiter → meter
            this._preFXBus.chain(this.globalFilter, this.globalChorus, this.globalPhaser, this.globalSaturator, this.globalBitCrusher, this.globalDelay, this.globalReverb, this.globalStereoWidener, this.sidechainGain, // ← FIX: was dangling in v2
            this.masterCompressor, this.masterLimiter, this.masterMeter);
            ...
            // ── Per-track nodes ──────────────────────────────────────────────────
            const built = await Promise.all(Array.from({ length: TRACK_COUNT }, (_, i) => this._buildTrack(i)));
            this.tracks.push(...built);
            // ── LFO nodes ────────────────────────────────────────────────────────
            this._initLFOs();
            // ── Schedulers ───────────────────────────────────────────────────────
    ...
    async _buildTrack(i) {
        ...
        const clips = Array.from({ length: CLIP_COUNT }, (_, ci) => ({
    ...
    _lfoSyncFreq(note) {
        if (!_Tone)
            return 1;
        try {
            const seconds = _Tone.Time(note).toSeconds();
            return 1 / seconds;
        }
        catch {
            return 1;
    ...
    _wireLFO(lfo) {
        ...
        const newScaleNodes = [];
        const connectToParam = (param, scale) => {
        ...
        switch (lfo.target) {
            case 'filter':
                connectToParam(this.globalFilter.frequency, 4000 * depth);
                break;
            case 'reverb':
                connectToParam(this.globalReverb.wet, 0.5 * depth);
                break;
            case 'delay':
                connectToParam(this.globalDelay.wet, 0.5 * depth);
                break;
            case 'volume':
                if (trackI !== null && this.tracks[trackI]) {
                    connectToParam(this.tracks[trackI].outputGain.gain, 0.4 * depth);
                }
                break;
            case 'pan':
                if (trackI !== null && this.tracks[trackI]) {
                    connectToParam(this.tracks[trackI].panner.pan, depth);
                }
                break;
            case 'chorus':
                connectToParam(this.globalChorus.wet, 0.6 * depth);
                break;
            case 'drive':
                connectToParam(this.globalSaturator.wet, 0.8 * depth);
                break;
            case 'bitcrush':
    ...
    setMidiClockOutput(enabled) {
        if (!_Tone || !this._midiOutput)
            return;
        if (enabled) {
    ...
    _onMidiMessage(e) {
        ...
        if (type === 0x90 && vel > 0) {
            ...
            if (trackIdx !== undefined)
                this.emit('midiNoteOn', trackIdx, note, vel);
            return;
        }
        ...
        if (type === 0x80 || (type === 0x90 && vel === 0)) {
            const trackIdx = this._midiNoteMap[note];
            if (trackIdx !== undefined)
                this.emit('midiNoteOff', trackIdx, note);
            return;
        }
        // ── Control Change ────────────────────────────────────────────────────
        if (type === 0xB0) {
            ...
            switch (note) {
                ...
                case 74: // Filter cutoff (MIDI standard)
                    if (this.globalFilter)
                        this.globalFilter.frequency.rampTo(200 + norm * 19800, 0.05);
                    break;
                ...
                case 91: // Reverb send → global reverb wet
                    if (this.globalReverb)
                        this.globalReverb.wet.rampTo(norm, 0.05);
                    break;
                case 93: // Chorus send → global chorus wet
                    if (this.globalChorus)
                        this.globalChorus.wet.rampTo(norm, 0.05);
                    break;
    ...
    // ── Player sync helpers ───────────────────────────────────────────────────
    startPlayerOnTransport(i) {
        const t = this.track(i, 'startPlayerOnTransport');
        if (!t || !_Tone || t._playerSynced || !t.player.loaded)
            return;
    ...
    stopPlayerFromTransport(i) {
        const t = this.track(i, 'stopPlayerFromTransport');
        if (!t || !_Tone || !t._playerSynced)
            return;
        try {
            t.player.stop();
            t.player.unsync();
    ...
    pushUndo(i) {
        ...
        if (!t || !t.player.loaded)
            return;
        const copy = t.player.buffer.get();
        if (!copy)
            return;
        if (t._undoStack.length >= UNDO_DEPTH)
            t._undoStack.shift();
        if (_Tone) {
    ...
    // ── Track FX controls ────────────────────────────────────────────────────
    setTrackVolume(i, vol) {
        const t = this.track(i, 'setTrackVolume');
        if (t)
            lerpParam(t.outputGain.gain, clamp(vol, 0, 1.5));
    }
    setTrackInputGain(i, gain) {
        const t = this.track(i, 'setTrackInputGain');
        if (t)
            lerpParam(t.inputGain.gain, clamp(gain, 0, 4));
    }
    setTrackPan(i, pan) {
        const t = this.track(i, 'setTrackPan');
        if (t)
            lerpParam(t.panner.pan, clamp(pan, -1, 1));
    }
    setTrackEQ(i, band, val) {
        const t = this.track(i, 'setTrackEQ');
        if (t)
            t.eq[band].value = clamp(val, -24, 24);
    }
    setTrackCompressor(i, threshold, ratio, attack = 0.003, release = 0.15) {
    ...
    setTrackGate(i, threshold) {
        const t = this.track(i, 'setTrackGate');
        if (t)
            t.gate.threshold = clamp(threshold, -80, 0);
    }
    setTrackSaturation(i, amount) {
    ...
    setTrackChorus(i, depth, freq, wet) {
        const t = this.track(i, 'setTrackChorus');
        if (!t)
            return;
        t.chorus.depth = clamp(depth, 0, 1);
        t.chorus.frequency = clamp(freq, 0.1, 10);
        lerpParam(t.chorus.wet, clamp(wet, 0, 1));
    }
    setTrackFlanger(i, depth, freq, wet) {
        const t = this.track(i, 'setTrackFlanger');
        if (!t)
            return;
        t.flanger.depth = clamp(depth, 0, 1);
        t.flanger.frequency = clamp(freq, 0.05, 5);
        lerpParam(t.flanger.wet, clamp(wet, 0, 1));
    }
    setTrackPhaser(i, freq, wet) {
        const t = this.track(i, 'setTrackPhaser');
        if (!t)
            return;
        t.phaser.frequency = clamp(freq, 0.1, 20);
        lerpParam(t.phaser.wet, clamp(wet, 0, 1));
    }
    setTrackBitCrusher(i, bits) {
        const t = this.track(i, 'setTrackBitCrusher');
        if (t)
            t.bitCrusher.bits = clamp(Math.round(bits), 1, 16);
    }
    setTrackTremolo(i, freq, depth, wet) {
        const t = this.track(i, 'setTrackTremolo');
        if (!t)
            return;
        t.tremolo.frequency = clamp(freq, 0.1, 20);
        t.tremolo.depth = clamp(depth, 0, 1);
        lerpParam(t.tremolo.wet, clamp(wet, 0, 1));
    }
    setTrackPitch(i, semitones, cents = 0) {
        const t = this.track(i, 'setTrackPitch');
        if (!t)
            return;
        t.pitchShift.pitch = clamp(semitones + cents / 100, -24, 24);
    }
    setHarmonyMode(i, mode) {
    ...
    setReverbSend(i, amount) {
        const t = this.track(i, 'setReverbSend');
        if (!t)
            return;
        t.reverbSendAmount = amount;
        lerpParam(t.reverbSend.gain, clamp(amount, 0, 1));
    }
    setDelaySend(i, amount) {
        const t = this.track(i, 'setDelaySend');
        if (!t)
            return;
        t.delaySendAmount = amount;
        lerpParam(t.delaySend.gain, clamp(amount, 0, 1));
    }
    setChorusSend(i, amount) {
        const t = this.track(i, 'setChorusSend');
        if (!t)
            return;
        t.chorusSendAmount = amount;
        lerpParam(t.chorusSend.gain, clamp(amount, 0, 1));
    }
    // ── Mute / Solo ───────────────────────────────────────────────────────────
    muteTrack(i, muted) {
    ...
    _updateSoloMuteBus() {
        this.tracks.forEach(t => {
            ...
            lerpParam(t.outputGain.gain, hear ? 1 : 0, 0.02);
        });
    }
    cueTrack(i, cued) {
    ...
    setGlobalFilter(freq) {
        if (!this.ready())
            return;
        lerpParam(this.globalFilter.frequency, clamp(freq, 20, 20000), 0.1);
    }
    setFilterType(type) {
        if (this.ready())
    ...
    setFilterResonance(q) {
        if (!this.ready())
            return;
        lerpParam(this.globalFilter.Q, clamp(q, 0.1, 20), 0.1);
    }
    setGlobalDelay(time, feedback, wet = 1) {
        if (!this.ready())
            return;
        this.globalDelay.delayTime.value = time;
        lerpParam(this.globalDelay.feedback, clamp(feedback, 0, 0.95));
        lerpParam(this.globalDelay.wet, clamp(wet, 0, 1));
    }
    setGlobalReverb(decay, wet) {
        if (!this.ready())
            return;
        lerpParam(this.globalReverb.wet, clamp(wet, 0, 1), 0.1);
        if (Math.abs(this.globalReverb.decay - decay) > 0.1) {
            this.globalReverb.decay = clamp(decay, 0.1, 30);
            void this.globalReverb.generate();
        }
    }
    setGlobalChorus(depth, rate, wet) {
        if (!this.ready())
            return;
        this.globalChorus.depth = clamp(depth, 0, 1);
        this.globalChorus.frequency = clamp(rate, 0.1, 10);
        lerpParam(this.globalChorus.wet, clamp(wet, 0, 1));
    }
    setGlobalPhaser(freq, wet) {
        if (!this.ready())
            return;
        this.globalPhaser.frequency = clamp(freq, 0.1, 20);
        lerpParam(this.globalPhaser.wet, clamp(wet, 0, 1));
    }
    setGlobalDrive(amount) {
        if (!this.ready())
            return;
        this.globalSaturator.distortion = clamp(amount, 0, 1);
        this.globalSaturator.wet.value = amount > 0.01 ? 1 : 0;
    }
    setGlobalBitCrusher(bits) {
        if (!this.ready())
            return;
        this.globalBitCrusher.bits = clamp(Math.round(bits), 1, 16);
    }
    setGlobalStereoWidth(width) {
        if (!this.ready())
            return;
        this.globalStereoWidener.width.value = clamp(width, 0, 1);
    }
    setMasterVolume(vol) {
        if (!this.ready())
            return;
        lerpParam(this.masterBus.gain, clamp(vol, 0, 1.5));
    }
    setMasterCompressor(threshold, ratio) {
        if (!this.ready())
            return;
     ...
     */
    enableMultiband(enabled) {
        if (!this.ready('enableMultiband') || !_Tone)
            return;
        if (this._mbEnabled === enabled)
            return;
        ...
        if (enabled) {
            ...
            try {
                this.masterBus.disconnect(this._preFXBus);
            }
            catch { /* ok */ }
     ...
     */
    setMultibandBand(band, threshold, ratio, gainDb = 0) {
        if (!this.ready('setMultibandBand'))
            return;
        ...
        if (gain) {
            lerpParam(gain.gain, this._dbToLinear(state.gain));
        }
    }
    /**
     ...
     */
    setMultibandCrossover(lowFreq, highFreq) {
        if (!this.ready('setMultibandCrossover'))
            return;
        ...
        if (!this._mbEnabled)
            return;
        if (this._mbLowLP)
            this._mbLowLP.frequency.value = this._mbState.lowFreq;
        if (this._mbMidHP)
            this._mbMidHP.frequency.value = this._mbState.lowFreq;
        if (this._mbMidLP)
            this._mbMidLP.frequency.value = this._mbState.highFreq;
        if (this._mbHighHP)
            this._mbHighHP.frequency.value = this._mbState.highFreq;
    }
    getMultibandState() {
     ...
     */
    setHarmonicExciter(amount, tone, drive) {
        if (!this.ready('setHarmonicExciter') || !_Tone)
            return;
        ...
        if (!this._exciterEnabled) {
            ...
            try {
                this.globalSaturator.disconnect(this.globalBitCrusher);
            }
            catch { /* ok */ }
            // Wire dry path
            ...
            // Update values only
            if (this._exciterHPF)
                this._exciterHPF.frequency.value = hpfFreq;
            if (this._exciterSat)
                this._exciterSat.distortion = satDrive;
            if (this._exciterWetGain)
                lerpParam(this._exciterWetGain.gain, wetAmt, 0.05);
            if (this._exciterDryGain)
                lerpParam(this._exciterDryGain.gain, 1 - wetAmt * 0.5, 0.05);
        }
     ...
     */
    bypassExciter(bypassed) {
        if (!this.ready('bypassExciter') || !this._exciterEnabled)
            return;
        if (bypassed) {
            if (this._exciterWetGain)
                this._exciterWetGain.gain.setTargetAtTime(0, ctx.currentTime, 0.015);
            if (this._exciterDryGain)
                this._exciterDryGain.gain.setTargetAtTime(1, ctx.currentTime, 0.015);
        }
        else {
            if (this._exciterWetGain)
                lerpParam(this._exciterWetGain.gain, this._exciterState.amount, 0.05);
            if (this._exciterDryGain)
                lerpParam(this._exciterDryGain.gain, 1 - this._exciterState.amount * 0.5, 0.05);
        }
    }
    getExciterState() { return { ...this._exciterState }; }
    _disposeExciterNodes() {
        try {
            this.globalSaturator.disconnect(this._exciterHPF);
        }
        catch { /* ok */ }
        try {
            this.globalSaturator.disconnect(this._exciterDryGain);
        }
        catch { /* ok */ }
        try {
            this._exciterSumBus?.disconnect(this.globalBitCrusher);
        }
        catch { /* ok */ }
        ...
        try {
            this.globalSaturator.connect(this.globalBitCrusher);
        }
        catch { /* ok */ }
        this._exciterEnabled = false;
    }
     ...
     */
    enableSidechain(sourceTrackIndex, amount, attack = 0.003, release = 0.15) {
        if (!this.ready('enableSidechain') || !_Tone)
            return;
        const t = this.tracks[sourceTrackIndex];
        if (!t) {
            console.warn(`[LoopEngine] enableSidechain: track ${sourceTrackIndex} not found`);
            return;
        }
        ...
        this._sidechainScheduleId = _Tone.Transport.scheduleRepeat((time) => {
            _Tone.getDraw().schedule(() => {
                ...
                const waveform = src.analyser.getValue();
    ...
    disableSidechain() {
        ...
        if (this.sidechainGain)
            lerpParam(this.sidechainGain.gain, 1.0, 0.05);
     ...
     * @deprecated
     */
    setSidechainTrack(sourceTrackIndex, amount) {
     ...
     */
    setGranularParams(grainSize, grainCount, pitch) {
        if (!this.ready('setGranularParams'))
            return;
     ...
     */
    setGranularFreeze(frozen, trackIndex = 0) {
        if (!this.ready('setGranularFreeze') || !_Tone)
            return;
    ...
    _startGrainScheduler() {
        ...
        this._granularScheduleId = _Tone.Transport.scheduleRepeat((time) => {
            if (!_Tone || !this._granularFrozen)
                return;
            const src = this.tracks[this._granularSourceTrack];
            if (!src?.player.loaded)
                return;
            ...
            for (let g = 0; g < this._grainState.grainCount; g++) {
                ...
                catch { /* ok */ } }, (gs + staggerSec + 0.5) * 1000);
            }
        }, GRAIN_INTERVAL);
    }
    _stopGrainScheduler() {
    ...
    isGranularFrozen() { return this._granularFrozen; }
    getGranularState() { return { ...this._grainState }; }
    /** Density — maps to grain interval scaling (0=sparse/slow, 0.5=normal, 1=dense/fast). */
    setGranularDensity(density) {
        if (!this.ready('setGranularDensity'))
            return;
     ...
     */
    setGranularSpread(spread) {
        if (!this.ready('setGranularSpread') || !_Tone)
            return;
        ...
        if (!this._granularWidener) {
            ...
            this._granularWidener.width.value = width;
        }
    }
    // ── Clip launcher ─────────────────────────────────────────────────────────
    async loadClip(trackIndex, clipIndex, buffer) {
    ...
    launchClip(trackIndex, clipIndex) {
        const t = this.track(trackIndex, 'launchClip');
        if (!t || !_Tone)
            return;
        const clip = t.clips[clipIndex];
        if (!clip?.hasContent || !clip._player)
            return;
        t.clips.forEach((c, ci) => {
            if (ci !== clipIndex && c.state === 'playing')
                this.stopClip(trackIndex, ci);
        });
        if (!clip._synced) {
    ...
    setClipMeta(trackIndex, clipIndex, name, color) {
    ...
    setBeatRepeat(config) {
        ...
        this._beatRepeat._scheduleId = _Tone.Transport.scheduleRepeat((time) => {
            if (Math.random() > config.chance)
                return;
            const dur = _Tone.Time(config.division).toSeconds() * config.length;
            ...
            catch { /* ok */ } }, (dur + 0.5) * 1000);
        }, config.division);
        this.emit('beatRepeatStart', config.trackIndex);
    }
    setBeatRepeatPitch(semitones) {
        if (!this.ready('setBeatRepeatPitch'))
            return;
    ...
    getLFOs() { return this._lfos; }
    // ── Macro controls ────────────────────────────────────────────────────────
    setMacro(id, value) {
        if (id < 0 || id >= MACRO_COUNT)
            return;
    ...
    setMacroTarget(id, target) {
        if (id < 0 || id >= MACRO_COUNT)
            return;
        this._macros[id].target = target;
    }
    setMacroLabel(id, label) {
        if (id < 0 || id >= MACRO_COUNT)
            return;
    ...
    _applyMacro(m) {
        ...
        switch (m.target) {
            case 'filter':
                this.setGlobalFilter(20 + v * 19980);
                break;
            case 'reverb':
                this.setGlobalReverb(2.5, v);
                break;
            case 'delay':
                this.setGlobalDelay('8n', v * 0.9, v);
                break;
            case 'drive':
                this.setGlobalDrive(v);
                break;
            case 'chorus':
                this.setGlobalChorus(v, 2, v);
                break;
            case 'bitcrush':
                this.setGlobalBitCrusher(1 + v * 15);
                break;
            case 'master_vol':
                this.setMasterVolume(v * 1.5);
                break;
            case 'bpm':
                this.setBpm(80 + v * 120);
                break;
            case 'xy_x':
                this.setGlobalFilter(200 + v * 15000);
                break;
            case 'xy_y':
                this.setGlobalReverb(0.5 + v * 10, v);
                break;
        }
    }
    getMacros() { return this._macros; }
    // ── Time / Quantize / Swing ───────────────────────────────────────────────
    ...
    getQuantMode() { return this._quantMode; }
    // ── Reverb pre-delay ──────────────────────────────────────────────────────
    /**
    ...
    setReverbPreDelay(seconds) {
        ...
        if (!this._reverbPreDelayNode) {
            ...
            this.tracks.forEach(t => {
                try {
                    t.reverbSend.disconnect(this.globalReverb);
                }
                catch { /* ok */ }
            ...
            this._reverbPreDelayNode.delayTime.value = delayTime;
        }
    }
     ...
     */
    setMono(enabled) {
        if (!this.ready('setMono'))
            return;
        if (this._monoEnabled === enabled)
            return;
        this._monoEnabled = enabled;
        if (enabled) {
            this._preMonoWidth = this.globalStereoWidener.width?.value ?? 0.5;
            this.globalStereoWidener.width.value = 0;
        }
        else {
            this.globalStereoWidener.width.value = this._preMonoWidth;
        }
    }
    /**
    ...
    enableLimiter(enabled) {
        ...
        if (enabled) {
            ...
            this._preBypassLimiterThreshold = this.masterLimiter.threshold.value;
    ...
    getTrackLevel(i) {
        ...
        const level = typeof v === 'number' ? v : v[0] ?? 0;
        if (level >= 0.99 && !t.clipHeld) {
            t.clipHeld = true;
            this.emit('clipDetected', i);
        }
        return level;
    }
    getStereoLevel(i) {
    ...
    getMasterLufs() { return rmsToLufs(this.getMasterLevel()); }
    getTrackWaveform(i) {
        const t = this.track(i);
        if (!t)
            return new Float32Array(ANALYSER_SIZE);
        return t.analyser.getValue();
    }
    getTrackFft(i) {
        const t = this.track(i);
        if (!t)
            return new Float32Array(FFT_SIZE / 2);
        return t.fft.getValue();
    }
    getMasterFft() {
        if (!this.ready())
            return new Float32Array(FFT_SIZE / 2);
    ...
    tapTempo() {
        ...
        if (this._tapTimes.length > TAP_HISTORY + 1)
            this._tapTimes.shift();
        if (this._tapTimes.length < 2)
            return this._pendingBpm;
        let total = 0;
    ...
    resetTapTempo() { this._tapTimes = []; }
    // ── Metronome ─────────────────────────────────────────────────────────────
    setMetronome(on, volume = 0.4) {
        this._metronomeOn = on;
        this._metronomeVol = volume;
        if (this.metronomeClick)
            this.metronomeClick.volume.value = -22 + volume * 12;
        if (this.metronomeAccent)
            this.metronomeAccent.volume.value = -18 + volume * 12;
    }
    // ── Transport ─────────────────────────────────────────────────────────────
    startTransport() {
        if (!this.ready() || !_Tone)
    ...
    stopTransport() {
        ...
        if (this._reverbPreDelayNode) {
            try {
                this._reverbPreDelayNode.dispose();
            }
            catch { /* ok */ }
            this._reverbPreDelayNode = null;
        }
    ...
    getTransportPosition() {
        if (!this.initialized || !_Tone)
            return '0:0:0';
        return _Tone.Transport.position;
    }
    getTransportSeconds() {
        if (!this.initialized || !_Tone)
            return 0;
    ...
    getCurrentBar() { return this._currentBar; }
    getCurrentBeat() { return this._currentBeat; }
    scheduleRepeat(cb, interval) {
        if (!this.ready('scheduleRepeat') || !_Tone)
            return -1;
        return _Tone.Transport.scheduleRepeat(cb, interval, 0);
    }
    clearSchedule(id) {
        if (id >= 0 && _Tone)
            _Tone.Transport.clear(id);
    }
    // ── Scene snapshots ───────────────────────────────────────────────────────
    captureScene(id, label, color = 'var(--dj-dimmer)') {
        const scene = {
            id, label, color,
            bpm: this.getBpm(),
            tracks: this.tracks.map(t => ({
                volume: t.outputGain.gain.value,
                ...
                eq: {
                    low: t.eq.low.value,
                    mid: t.eq.mid.value,
                    high: t.eq.high.value,
                },
                harmonyMode: 'off',
                playbackMode: t.playbackMode,
                inputGain: t.inputGain.gain.value,
                saturation: t.saturator.distortion,
                compThreshold: t.compressor.threshold.value,
                compRatio: t.compressor.ratio.value,
                chorus: {
                    depth: t.chorus.depth,
                    rate: t.chorus.frequency?.value ?? 2,
                    wet: t.chorus.wet.value,
                },
                phaser: {
                    frequency: t.phaser.frequency?.value ?? 0.5,
                    wet: t.phaser.wet.value,
                },
                bitDepth: t.bitCrusher.bits,
                tremolo: {
                    frequency: t.tremolo.frequency?.value ?? 4,
                    depth: t.tremolo.depth ?? 0,
                    wet: t.tremolo.wet.value,
                },
            })),
            fx: {
                filterFreq: this.globalFilter.frequency.value,
                filterType: this.globalFilter.type,
                filterRes: this.globalFilter.Q.value,
                reverbWet: this.globalReverb.wet.value,
                reverbDecay: this.globalReverb.decay,
                delayFeedback: this.globalDelay.feedback.value,
                delayTime: String(this.globalDelay.delayTime.value),
                driveAmount: this.globalSaturator.distortion,
                stereoWidth: this.globalStereoWidener.width?.value ?? 0.5,
                phaserWet: this.globalPhaser.wet.value,
                bitDepth: this.globalBitCrusher.bits,
    ...
    recallScene(scene) {
        ...
        this.setGlobalDelay(scene.fx.delayTime, scene.fx.delayFeedback);
    ...
    // ── Scene morph ───────────────────────────────────────────────────────────
    morphScenes(from, to, durationMs = 2000) {
        ...
        const tick = () => {
            ...
            this.setGlobalReverb(lerp(from.fx.reverbDecay, to.fx.reverbDecay, t), lerp(from.fx.reverbWet, to.fx.reverbWet, t));
            ...
            from.tracks.forEach((fs, i) => {
                ...
                this.setTrackChorus(i, lerp(fs.chorus.depth, ts.chorus.depth, t), lerp(fs.chorus.rate, ts.chorus.rate, t), lerp(fs.chorus.wet, ts.chorus.wet, t));
            });
    ...
    cancelMorph() { cancelAnimationFrame(this._morphRafId); }
    // ── Guards ────────────────────────────────────────────────────────────────
    ready(caller) {
        if (!this.initialized || !_Tone) {
            if (caller)
                console.warn(`[LoopEngine] ${caller}() called before init().`);
            return false;
        }
        return true;
    }
    track(index, caller) {
        if (!this.ready(caller))
            return null;
        const t = this.tracks[index];
        if (!t) {
            console.warn(`[LoopEngine] track(${index}) out of range.`);
            return null;
        }
        return t;
    }
    ...
    dispose() {
        ...
        if (this._beatRepeat._scheduleId >= 0)
            _Tone.Transport.clear(this._beatRepeat._scheduleId);
        if (this._beatRepeat._buffer)
            this._beatRepeat._buffer.dispose();
        // Granular
        this._stopGrainScheduler();
        if (this._granularWidener) {
            try {
                this._granularWidener.dispose();
            }
            catch { /* ok */ }
            this._granularWidener = null;
        }
...
// ── Public API ────────────────────────────────────────────────────────────────
let engine = null;
export function getLoopEngine() {
    if (!engine)
        engine = LoopEngine.getInstance();
    return engine;
}
export { LoopEngine };
