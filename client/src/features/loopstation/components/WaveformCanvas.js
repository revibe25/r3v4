...
// ── IDLE LINE ─────────────────────────────────────────────────────────────────
function drawIdleLine(ctx, w, h, color, t) {
    const mid = h / 2;
...
// ── MODE: WAVEFORM ────────────────────────────────────────────────────────────
function drawWaveform(ctx, data, w, h, color, lw, gain) {
...
// ── MODE: MIRROR ──────────────────────────────────────────────────────────────
function drawMirror(ctx, data, w, h, color, lw, gain) {
    const mid = h / 2;
    const step = w / data.length;
...
// ── MODE: SPECTRUM ────────────────────────────────────────────────────────────
const peakHold = new Float32Array(512);
const peakDecay = 0.004;
function drawSpectrum(ctx, data, w, h, color) {
    ...
    for (let i = 0; i < bins; i++) {
        ...
        if (n > peakHold[i])
            ...
            peakHold[i] = Math.max(0, peakHold[i] - peakDecay);
...
// ── MODE: DOTS ────────────────────────────────────────────────────────────────
function drawDots(ctx, data, w, h, color, gain) {
...
function drawLissajous(ctx, data, w, h, color, gain) {
    if (data.length < 4)
        return;
    const cx = w / 2, cy = h / 2;
...
function drawWaterfall(ctx, data, w, h, color) {
...
function drawCircular(ctx, data, w, h, color, lw, gain) {
    const cx = w / 2, cy = h / 2;
...
let terrainHistory = [];
function drawTerrain(ctx, data, w, h, color) {
    // Push new row
    const bins = Math.min(data.length, 128);
    const row = new Float32Array(bins);
    for (let i = 0; i < bins; i++)
        row[i] = Math.max(0, (data[i] + 100) / 100);
    terrainHistory.push(row);
    if (terrainHistory.length > TERRAIN_ROWS)
...
// ── EMA Smoother ──────────────────────────────────────────────────────────────
function smoothData(data, prev, alpha) {
    if (!prev || prev.length !== data.length || alpha <= 0)
        return data;
...
// ── Component ─────────────────────────────────────────────────────────────────
export const WaveformCanvas = ({ trackIndex, isActive, mode = 'mirror', color = 'var(--looper-acid-2)', background = 'transparent', lineWidth = 1.5, className = '', width = 64, height = 64, persistence = 0.3, gain = 1.0, smoothing = 0, beatFlash = false, }) => {
    ...
    return (_jsx("canvas", { ref: canvasRef, width: width, height: height, className: className, style: { width, height, display: 'block' }, "aria-label": `Track ${trackIndex + 1} ${mode} visualiser` }));
};
