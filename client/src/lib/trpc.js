 ...
 */
import { createTRPCReact, createTRPCProxyClient, httpBatchLink, loggerLink, } from '@trpc/react-query';
...
export const trpcVanilla = createTRPCProxyClient({
    links: [
        loggerLink({
            enabled: opts => import.meta.env?.DEV === true &&
...
export function invalidateDAWQueries() {
    queryClient.invalidateQueries({ queryKey: [['daw', 'project.list']] });
}
// ── Error classifiers ─────────────────────────────────────────────────────────
export function isTRPCForbidden(err) {
    return err?.data?.code === 'FORBIDDEN';
}
export function isTRPCUnauthorized(err) {
